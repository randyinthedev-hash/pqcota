...
**구조를 그림으로** — 세 단계가 무엇을 주고받는지, 그리고 그 바깥에서 누가(CMDB·운영자·provider 공급처·대상 노드) 무엇을 대는지를 한 장에 담았다. 어디까지가 이 도구의 실행 경계인지도 함께 표시된다 → [플랫폼 구조도](docs/architectures/platform-structure.html)(브라우저로 열면 한/영 전환된다).
