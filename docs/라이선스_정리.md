...
격리 3원칙(별도 프로세스·표준 데이터만 교환·배포 분리)은 **규정서 §6.2**가 정한다. 여기서는
그것이 코드·빌드에서 어떻게 강제되는지를 적는다.

| 원칙 | 무엇이 강제하나 |
|---|---|
| 별도 프로세스 | `contracts/.../collector.proto`의 intake 계약(§6.1) — GPL collector는 gRPC/CLI 뒤에 선다 |
...
**사용자가 고를 때 함의를 보인다**(§6.3): collector 선택 UI는 `CollectorCapabilities.license`로
