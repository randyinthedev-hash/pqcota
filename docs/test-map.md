...
위 수는 손으로 적은 것이 아니라 리포에서 센 값이고, **`check-docs`가 그 말이 참인지 대신 센다.** unit과 integration이 어긋나면 게이트가 막는다. e2e만은 세지 않는데, Go 테스트가 아니라 데모 6단계라서 셀 대상이 없기 때문이다. 직접 확인하려면 아래를 돌린다:
