...
**문서 성격**: [프로세스 규정서](PQC플랫폼_단계별_프로세스규정.md)를 코드 레벨로 구현하기 위한 아키텍처 설계.
프로세스 규정서가 "무엇을·어떤 규칙으로(WHAT)"라면, 이 문서는 "어떤 모듈·인터페이스·스키마로(HOW)"를 확정한다.
**기준 규정서 버전**: **v4** (단계적 배포 모델 §4.5, provider 시그니처 레지스트리 §2.3, 규제 자산 FIPS 라우팅 §4.10 반영).
...
> **규정서 핵심의 아키텍처 영향**:
> - **§4.5 단계적 배포 모델** = 부트스트랩 provider(얇은 로더, 사전 설치) + PQC 페이로드(런타임 배포) 2-컴포넌트. `deploy_automation_level`(L1/L2/L3) 1급 속성. L3의 활성화·재시작은 계획의 `activation` 훅으로 플레이북에 들어간다(방법을 도구가 추측하지 않는다). 부트스트랩 provider 채널은 다루지 않는다. 아래 §2.1 모듈 맵에 반영.
...
| 정적 ELF 심볼·문자열 시그니처로 fork·version 판별 (§2.3, §2.4) | ELF 파서. Go(`debug/elf`)·Rust(`goblin`) 둘 다 강함 |
| eBPF 동적 트레이스 (§2.2 crypto-tracer, §2.4 dynamic-trace) | **eBPF 로더 생태계 = Go(cilium/ebpf)가 사실상 표준** |
| **JVM attach → `Security.getProviders()` 실체 조회 (§2.2, §2.3)** | **JVM 내부에서만 가능 — JVM 강제(플랫폼 언어 Java). 우회 불가** |
| CycloneDX CBOM(ECMA-424) 입출력 (§2.5, §3.2) | 성숙 라이브러리 필요. JVM·JS·Go 순으로 성숙 |
| Ansible/Salt substrate 오케스트레이션 (§4.6) | 서브프로세스·SSH. 언어 무관, Go 편함 |
...
| **런타임 간 계약** | **gRPC + Protobuf** (+ CLI/stdout 폴백) | intake 계약(§6.1)·서브프로세스 격리(§6.2)를 동일 메커니즘으로 |
...
- **배포 단순성**: 온호스트 에이전트가 단일 정적 바이너리 → 규정서 §4.6 "자기잠금 회피"에 유리.
- **오케스트레이션·클라우드네이티브 정렬**: Ansible/Salt 서브프로세스·mTLS·gRPC 관성.
- **Rust는 어디에?** ELF 심볼 분석기(§2.3)는 순수하고 고립된 모듈이라 **나중에 Rust로 교체 가능한 명확한 후보**. intake 계약 뒤에 숨으므로 코어 무관하게 스왑된다. 지금은 Go로 시작하고 필요 시 격상.
...
```
                         ┌─────────────────────────────────────────────┐
                         │  스코프 마스터 (CMDB/자산 등록부) — §0.4       │
                         │  관리 대상 경계의 유일 권위 소스. 모든 게이트   │
                         └───────────────────┬─────────────────────────┘
                                             │ (대상 노드 목록)
   ┌── DISCOVERY (§2) ───────────────────────▼─────────────────────────┐
   │                                                                    │
   │  [Collector Framework] ── intake 계약(§6.1) ─┐                      │
   ...
   │    CONFIRMED / UNDECLARED / UNOBSERVED          │ │  (§0.3)
   ...
   │    (§4.3/§4.4) · before 캡처 · 롤백 레코드 영속  │ │
   │  [Substrate Adapter] Ansible/Salt (L1·L2, §4.6) │ │
   └────────────────────────┬───────────────────────┘ │
              ╔═══════════════════════════════════╗   │
              ║ ③ 프로비저닝 히스토리 (행위 계열)  ║ ─┘
              ╚═══════════════════════════════════╝
                            │ 재스캔 폐루프 → Discovery (§5)
                            ▼
   [Provenance/Audit Service] — 4계열을 인과 사슬로 봉합, 불변 감사 로그 (§0.3, §4.9)
   [API Gateway] + [Web UI] — 리뷰 큐·인벤토리·감사 뷰
```

### 2.2 데이터 흐름의 불변식 (§0.2 강제)
...
**collector는 `CollectionResult`를 emit하는 CLI다**(`pqcota-nodescan`·`pqcota-jvmscan`·`pqcota-netcap`). 배포는 표준 substrate(Ansible)로 한다 — 디스커버리 실행 시 관측 대상 노드에 반입·실행·회수하고 잔재를 남기지 않는다([collector 배포 설계](../discovery/collector_배포_설계.md)). **자체 원격 실행 엔진은 만들지 않는다.**

- **이 리포**: collector CLI + **T1 self-service**(서명된 collector 번들을 사용자가 직접 실행 — 에어갭 포함) + **결과 서명·검증**(ed25519, `pqcota-keygen`·`PQCOTA_VERIFY_KEY`) + **스코프 마스터 게이트**(§0.4, `pqcota-ingest`가 등재 노드만 수용). collector를 사용자 자신의 substrate로 감싸 돌릴 수도 있다. 릴리스·번들 서명(공급망 위생)은 여기 속한다.
- **원칙(불변)**: 어느 경로든 **스코프 게이트 필수** + **RCE 대칭성**(레거시 호스트에 실행체 투입은 위험하므로 서명검증·최소권한·멱등). 부가가치는 push 채널 소유가 아니라 그 위의 게이트·서명·완전성 맵.

**호스트 발자국 (Phase 0 최소)** — [§1.2 스택] 근거와 직결:
- **OpenSSL 노드**: Go 정적 바이너리 1개 + root/`CAP_SYS_PTRACE` + mTLS 자격. 그 외 의존 0(ELF·/proc 자립 파싱, `ldd`/`lsof`/`ss`/`readelf` 비의존 설계).
- **Java 노드**: **Go 바이너리 + 인트로스펙션 agent JAR**만 올리면 된다 — attach는 OS IPC(트리거 파일+SIGQUIT+유닉스 소켓)라 **JDK 없이 직접** 붙는다(대상이 순수 JRE·jlink 런타임이어도). **동일 UID/root** 필요. HotSpot이 아니면(OpenJ9) 머신의 JDK를 클라이언트로 쓰는 경로로, 그마저 막히면(`DisableAttachMechanism`·JEP 451) 정적 경로로 열화 → `evidence_strength` 하향(§2.4). 3계층 상세: [jvm-collector README](../discovery/collectors/jvm/README.md).
...
{
  "envelope": {                          // §2.7 provenance — Envelope
    "collector_id": "openssl-collector",
    "collector_version": "0.1.0",
    "detection_method": "runtime-introspection", // §2.4 열거형(아래)
    "collected_at": "2026-07-07T05:00:00Z",
    "target_node_id": "cmdb://node/8f3a...", // 스코프 마스터 앵커 (§0.4)
    "signature": "ed25519:...",          // collector 서명 (§2.7 무결성)
  ...
  "completeness": {                       // §2.7 완전성 맵 — collector별·계층별 분리
    "layers_covered": ["artifact", "runtime-introspection"],
    "layers_missing": ["process"],        // "실제 없음" ≠ "원리상 못 봄" 구분 (§2.7)
    "note": "프로세스 계층 미수집 — 대상 미실행"
  }
}
```

### 3.2 Finding 스키마 (런타임 추상 — §1.3, §2.4)
...
type Finding struct {
    ...
    EvidenceStrength string      `json:"evidence_strength"`// confirmed|inferred-high|inferred-low
                                                           // detection_method에서 결정론적 파생 (§2.6 AUTO)
    // ─ 런타임별 분기 (§1.3) — oneof ─
...
type JCAAxes struct {
    ...
    ProviderSet      []string `json:"provider_set"`      // 등록 순서 포함 (우선순위 협상 판정 근거, §1.2)
    RegistrationMode string   `json:"registration_mode"` // static|dynamic|explicit
}
```

**`detection_method` → `evidence_strength` 매핑 (규정서 §2.4 표를 결정론적 함수로)**:

```go
// §2.6: evidence_strength 부착은 AUTO(결정론적). 이 함수가 유일 소스.
func EvidenceStrength(method string) string {
    ...
    default:                       return "unknown"        // §2.6 "unknown도 1급 증거"
    }
}
```

**불변 규칙 (§2.6)**: 채우지 못한 필드는 `nil`/빈값이 아니라 명시적 `"unknown"`. 자동 "부재" 처리 금지.

**v3/v4 스키마 반영 (필드가 어느 단계에 속하는가)**

- **`FipsValidation`는 이미 있음** — v3 §4.10 "규제 자산 FIPS 라우팅"에서 이 필드가 **provider 선택을 강제**한다. Discovery는 값을 채우기만 하고(강화 단계), 라우팅 판정은 하지 않는다. `JCAAxes.ProviderSet`을 crypto-registry(§2.3)와 대조해 `pqc_readiness`·`fips_validation`을 파생한다.
...
- **`deploy_automation_level`(L1/L2/L3)은 Finding 필드가 아니다** — Discovery 산출물이 아니라 **리뷰어가 자산별로 정하는 계획·자산 속성**(§4.5, MANUAL). 확정 계획(plan) 엔티티에 속한다. 단, 통제 어휘로서 SSOT(contracts)에는 등재한다(§3.3·contracts 참조).

### 3.3 provider 시그니처 레지스트리 (§2.3) — 강화 단계 참조 데이터

Discovery 강화(§2.5 step 3)가 참조하는 결정론적 매핑 테이블. **파생 규칙이므로 개선 시 원본에서 재계산**(§0.2), `ruleset_version`으로 버전 고정.
...
// LicenseClass="permissive"(BouncyCastle 표준판, MIT계열)는 GPL 격리 대상 아님 (§4.4).
```

### 3.4 PQC 알고리즘 성숙도 레지스트리 (§2.3 참조 데이터) — remediation 라우팅 입력

§3.3이 **provider의 능력**(어떤 알고리즘을 구현하는가)을 다룬다면, 이 절은 **알고리즘 자체의 표준화 성숙도**(그 알고리즘을 써도 되는가)를 다룬다. 관측된 협상 그룹(`negotiated_group`, network-collector)·provider 알고리즘 이름을 이 표와 대조해 성숙도를 파생하고, §4.10 remediation 분기의 입력으로 쓴다. **파생 규칙**(§0.2)이며 이 리포의 공개 참조 데이터(`pkg/kernel/registry`).
...
## 4. Collector Intake 계약 (§6.1 — 플러그형 인터페이스)
...
message CollectorCapabilities {
  ...
  repeated string detection_methods = 5; // §2.4 열거형
  string license = 6;                    // "Apache-2.0" | "GPL-3.0-or-later" ← §6.3 UX 표기용
}

message CollectRequest {
  repeated string target_node_ids = 1;   // 스코프 마스터 게이트 통과분만 (§0.4)
...
1. **출력은 항상 정규화된 CBOM Envelope** — 어느 백엔드든 downstream(reconcile·리뷰·프로비저닝)이 백엔드 무관하게 동일 동작(§6.1).
2. **스코프 마스터 게이트는 코어 책임** — Collector에 넘기기 전 코어가 대상 노드를 필터(§0.4). Collector는 받은 노드만 수집.
3. **완전성은 계층별로 신고** — Collector가 `Describe`로 커버 계층을 선언하고 `Collect`로 실제 커버를 보고. "못 본 것"을 코어가 갭으로 기록(§2.7).

### 4.3 GPL 격리와 동일 메커니즘 (§6.2)

이 gRPC/CLI 경계가 곧 **GPL 전염 차단 경계**다. GPL collector(CipherIQ `cbom-generator` 등)는 **별도 프로세스로 실행 + stdout CycloneDX만 교환** → 라이브러리 링크 없음. `gpl-adapter`가 서브프로세스를 감싸 intake 계약으로 노출한다. → **§6.2 격리 3원칙이 코드에서 자동 성립.**

---

<a id="repo-scope"></a>

## 5. 라이선스 경계 (OSS · GPL 격리)

이 리포는 **Apache-2.0**(허용적, 전염 없음)로 공개한다. 관측성(디스커버리)을 오픈소스로 풀어 표준·생태계를 선점하는 것이 목표다. GPL 컴포넌트는 배포·의존을 분리해 전염을 차단한다.

### 5.1 이 리포가 담는 것

| 구분 | 라이선스 | 포함물 |
|---|---|---|
| **이 리포** | **Apache-2.0** (허용적) | intake 계약/SDK, 정규화된 CBOM 스키마·프로파일, **정규화 파이프라인**, **provider 시그니처 레지스트리**(§2.3, 공개 참조 데이터), **레퍼런스 Collector**(openssl, **jvm-introspection**, network), CLI, **중앙 인벤토리 뷰**(머신 메타데이터 저장소=엔드포인트·프로필 upsert·**앱 귀속**), **프로비저닝 생성**(L1/L2/L3 Ansible 플레이북·활성화 훅·before 캡처·**롤백 레코드 영속**) |
| **GPL 어댑터** (선택) | GPL-3.0(격리) | CipherIQ/CBOMkit 서브프로세스 래퍼 — §6.2 GPL 컴포넌트, 서브프로세스+CycloneDX 경계로 전염 차단 |
| **PQC provider 라이브러리** | 자산별 상이 | BouncyCastle(**허용적, 번들 가능**) / BC-FJA(FIPS, 별도 계약) / JDK 네이티브 / 내부 provider(OpenSSL·특수 케이스) — §4.4: Java 표준 PQC는 BC 채택, 내부 provider는 OpenSSL·HSM·독자 알고리즘으로 한정. BC 표준판은 GPL 격리 대상 아님 |

레퍼런스 Collector는 OSS로 유지한다 — §2.2 "자체 구현 공백"인 JVM 인트로스펙션을 공개해 커뮤니티를 끌어들인다. 런타임 collector가 막히면 **정적 경로로 정직 열화 + 완전성 맵 갭**으로 완결한다(§2.4). source 스캔은 CI 가시라 직접 만들지 않고 CBOMkit/CI에 위임 + cbom-import로 수신한다(§SV-2).

### 5.2 GPL 격리 규칙

이 gRPC/CLI 경계(§4.3)가 곧 GPL 전염 차단 경계다. GPL collector(CipherIQ `cbom-generator` 등)는 **별도 프로세스로 실행 + stdout CycloneDX만 교환** → 라이브러리 링크 없음. `gpl-adapter`가 서브프로세스를 감싸 intake 계약으로 노출한다(§6.2).

- `pqcota`는 `pqcota-collectors-gpl`을 **의존성으로 갖지 않는다**(go.mod에 없음). 사용자가 선택 설치, 런타임에 gRPC/CLI로만 발견.
- CI에서 라이선스 스캐너로 교차 의존 위반을 게이트(전염 차단 자동 검증).
...
pqcota/            # Apache-2.0 · 공개 · 전 범위(Discovery·인벤토리·프로비저닝 생성)
  ...
  ├─ pkg/               # 라이브러리 로직 — 단계 그룹:
  │    ├─ discovery/    #   관측 레인: normalize(§2.5)·history(§2.5⑥ 스냅샷 스토어)
  │    ├─ inventory/    #   ingest(적재·CBOM 수신 SV-2)·중앙 뷰(§5)·머신 메타데이터 저장소(엔드포인트·프로필 upsert)·hosts 파서 + declaration(선언 레인). 대조·판정 엔진은 없다
  │    ├─ provisioning/ #   확정계획 게이트(§3.7)·taxonomy→config 생성기(§4.3/§4.4)·L1/L2 플레이북·before 캡처·롤백 레코드 저장소. 생성·영속까지
  │    └─ kernel/       #   단계 가로지르는 공유 규칙: registry·posture·scope·machineid·sign
  ├─ discovery/         # 실행 진입점(단계별):
  │    ├─ collectors/{openssl(Go),jvm(Java 사이드카 ★),network(Go)}  # §6.1 플러그인·GPL 격리 경계
...
pqcota-collectors-gpl/  # GPL-3.0 · 별도 리포 · 절대 번들·링크 금지 (§6.2 배포 분리)
  └─ adapters/cipheriq/  adapters/cbomkit/   # 서브프로세스 어댑터만
```

### 5.4 사용자 선택 UX (§6.3)

CLI/UI에서 Collector 선택 시 `CollectorCapabilities.license`로 함의 명시:
`"CipherIQ 백엔드 = GPL 컴포넌트 별도 설치"` vs `"레퍼런스 Collector = Apache-2.0 포함"`. 선택 무관 정규화된 CBOM 수렴 보장.

> **주의(§6.2)**: 컴포넌트별 정확한 라이선스(GPL vs AGPL, or-later) 실사 필수. 배포 전 OSS 라이선스 전문 변호사 검토. 본 설계는 법률 자문이 아님.
...
|---|---|---|
| Collector intake 계약(protobuf) + SDK | §6.1 | — (계약) |
| `openssl-collector`: `/proc`·`ldd`·`readelf`·ELF 심볼 | §2.3 | AUTO |
| `jvm-collector`: JVM attach → `getProviders()` | §2.2, §2.3 ★ | AUTO(실행 중), 미실행은 갭 |
| 선언 임포트(CMDB/자산 등록부 로드) | §0.4, §7 | AUTO |
| 정규화 파이프라인 6단계 + `evidence_strength` 부착 | §2.5, §2.4 | AUTO |
| **provider 시그니처 레지스트리 강화**(JCA provider_set → pqc_readiness·fips·알고리즘, SLH-DSA 갭 태깅) | §2.3 v3 | AUTO |
| 완전성 맵(계층별) | §2.7 | AUTO |
| 디스커버리 히스토리(append-only) + **이력 열람·스냅샷 간 변화 diff**(관측 사실 서술이지 판정 아님 — §6 기준) | §2.5-6 | AUTO |
| **관측 기록/스냅샷 2층 분리** — 스냅샷은 실질 내용이 바뀔 때만, 관측 기록은 적재마다. 같은 상태 반복 관측이 저장을 늘리지 않되 "언제 봤나"는 보존 | §2.5-6, §0.2 | AUTO |
| **보존 정책 절단**(`pqcota-prune`) — 오래된 변화 지점 절단. 최신 불가침, 절단 사실을 기록으로 남겨 이력의 구멍을 고지 | §2.7 | — (사용자 지시) |
| **자산 스코프 게이트**(`scope.AssetPolicy`) — 노드 게이트(§0.4)를 자산 단위로. 사용자가 선언한 관리 대상만 적재, 제외 건수는 고지 | §0.4, §2.7 | AUTO(집행) |
| 중앙 인벤토리 뷰 (CLI+UI) + 머신 메타데이터(엔드포인트·프로필)·**앱 귀속** | §7 | — |
| 프로비저닝 생성 (확정계획 게이트 → L1/L2 플레이북 + before·**롤백 레코드**) | §4.3/§4.4 | — (생성만) |
...
- **PROPOSE 기본 비활성** — dynamic-trace(eBPF 침습, §2.6).
...
1. 스코프 마스터에 등재된 노드만 수집 대상이 된다(§0.4 게이트 동작).
...
4. 미실행 프로세스·미수집 계층이 **완전성 맵에 갭으로** 남는다("없음"으로 오판하지 않음, §2.7).
5. 전 과정이 디스커버리 히스토리에 append되고 재계산으로 재현된다(§0.2).

---

## 7. 다음 액션 (제안)

1. **`contracts/` 먼저 확정** — Collector intake protobuf + CBOM Envelope 스키마가 모든 것의 SSOT(§6.1). 이게 굳으면 Collector·코어를 독립 병렬 개발 가능.
