...
**핵심 관찰**: JVM 인트로스펙션(§2.2 "자체 구현 공백 영역")은 **어떤 언어로도 우회할 수 없고 반드시 JVM 안에서 실행**되어야 한다. 그래서 폴리글랏이 불가피하다. 나머지 시스템 수집은 Go 하나로 전부 맡는다.
...
- **오케스트레이션·계약 정렬**: Ansible/Salt를 서브프로세스로 부르고, 계약을 gRPC로 규격한다. 전송 인증(mTLS 등)은 그 substrate에 이미 있는 것을 쓴다.
- **Rust는 어디에?** ELF 심볼 분석기(§2.3)는 순수하고 고립된 모듈이라 **나중에 Rust로 교체 가능한 명확한 후보**. intake 계약만 지키면 되므로 코어를 건드리지 않고 갈아끼울 수 있다. 지금은 Go로 시작하고, 필요해지면 그때 바꾼다.
...
> **이 맵은 플랫폼 전체의 구상이고, 이 리포의 범위는 [§6](#6-무판단-원칙)이 정한다.**
...
**불변 규칙 (§2.5)**: 채우지 못한 필드는 `nil`/빈 값이 아니라 명시적 `"unknown"`. 자동 "부재" 처리 금지.
...
이 리포는 **관측·정규화·영속·생성**을 한다. 사실을 수집하고, 거기서 파생하고, 산출물을 만든다.
...
| `openssl-collector`: `/proc`·ELF 심볼 자체 파싱(`ldd`·`readelf` 비의존) | §2.3 | AUTO |
...
| **보존 정책 절단**(`pqcota-prune`)이다. 오래된 변화 지점을 자른다. 최신 불가침, 절단 사실을 기록으로 남겨 이력의 빈자리를 고지 | §2.6 |: (사용자 지시) |
