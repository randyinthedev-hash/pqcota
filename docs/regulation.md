...
이 규정 전체를 지배하는 원칙. 단계별 규정(§2–§4)은 모두 여기서 파생된다.
...
**자산관리정보(사용자 CMDB/자산 등록부)가 관리 대상 경계의 유일한 권위 소스다.** 디스커버리 배포, 인벤토리 등재, 프로비저닝 실행 대상이 모두 이 마스터로 게이트된다. 마스터 미등재 노드는 원칙적으로 수집·실행 대상이 아니며, 관측으로 발견되면 "실행 대상"이 아니라 "등재 판정 요청 대상"으로 흐른다.
