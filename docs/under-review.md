...
## 5. 엣지를 앱에 귀속시키는 일 (v0.4.0 예정)

> **하기로 정했다.** [로드맵](../RELEASE_NOTES.md#로드맵--예정-릴리스-계획)의 v0.4.0이다.
...
5.1은 계약을 건드리고 5.2는 새 입력 형식을 하나 더 만든다. 둘 다 되돌리기 어려운 종류라,
**Windows CNG로 substrate를 일반화한 뒤(v0.3.0)** 하는 편이 낫다 — 그때 파일·레지스트리 두 substrate를
다 본 상태에서 귀속 모델을 정하면, 나중에 다시 넓힐 일이 줄어든다.

그때까지는 재관측이 그 공백을 메운다. 한 앱을 바꾸고 엣지가 따라 바뀌는지는 **바꿔 보고 다시 관측하면**
알 수 있다 — 예측이 안 되는 것이지 확인이 안 되는 것은 아니다.
