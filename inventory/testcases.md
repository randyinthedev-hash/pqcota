...
| **[TV-HISTORY-10](../pkg/discovery/history/migrate_pg_test.go) ✅** | **v0.8 모양의 표(= `content_hash_v1` 없음)에 옛 행을 넣고 v0.9 코드로 연다** · 같은 상태를 적재 · 한 번 더 적재 | 열과 `(org, node_id, ruleset_ver, content_hash_v1)` 인덱스가 더해진다. **옛 행의 v1은 빈 채 보존**되고 소급하지 않는다. 첫 적재는 같은 상태여도 **새 행**, 두 번째는 접힌다. 그 행부터 참조가 찾힌다 | D8의 완료 조건이다. 옛 지문으로 계속 접으면 v1 이 빈 옛 행이 재사용되어 v1 열이 영원히 비고 다운스트림의 참조가 영원히 해결되지 않는다. 전용 스키마에서 돌아 공유 표의 모양을 흔들지 않는다. `PQCOTA_TEST_DSN`이 있을 때만 |
