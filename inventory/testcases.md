...
| [TV-HISTORY-9](../pkg/discovery/history/lookup_pg_test.go) | `TestPgDedupAndLookupOnV1`: Postgres에서 TV-HISTORY-8과 같은 절차 | 같다. `content_hash_v1` 열과 `(org, node_id, ruleset_ver, content_hash_v1)` 인덱스로 찾는다 | 메모리 저장소와 Postgres가 갈리면 로컬에서 찾히던 참조가 중앙에서 안 찾힌다. `PQCOTA_TEST_DSN`이 있을 때만 돈다 |
