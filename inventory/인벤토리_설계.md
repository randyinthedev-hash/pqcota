...
인벤토리가 관리하는 머신 정보는 **기술 식별(node_id·지문)과 분리**된 두 종류를 가진다 — 식별은 기계가 관측(디스커버리 §0.5), 이 둘은 **선언(CMDB)/리뷰어가 채우고 사용자가 수정**한다:

- **`MachineEndpoint`**(node_id·name·ip·port) — discovery 재접속용 **재사용 연결 메타데이터**. 사용자 hosts 파일에서 안전 부분집합만 적재(§0.5). **접근 비밀은 미적재**(타입에 비밀 필드 없음).
...
**앱 귀속(§0.5 자산 모델)** — 각 파생 `Finding`은 `app_keys`(복수)로 애플리케이션에 귀속되고 뷰에 `@app`으로 표시된다. host-wide 스캔에서 하나의 공유 라이브러리(예 `libcrypto.so.3`)를 여러 앱이 로드하면 **다중 귀속**된다 — 그 .so 교체의 영향 반경을 온전히 보이기 위함. 판단은 하지 않는다(§2.1).
...
### 6.4 한계 (Discovery §2.5 network-collector에서 상속)
...
- **공유 `.so`는 귀속 앱이 여럿**이라(§0.5) 그중 **하나만 맞아도** 규칙이 걸린다.
