...
**문서 성격**: 규정서 §3(Inventory)의 기술 설계. Discovery 산출(정규화된 CBOM·완전성 맵)을 **읽기전용 인벤토리 뷰**로 제시하고, 머신 메타데이터를 관리한다.
**기준**: 규정서 v4 §3·§5, [디스커버리 설계](../discovery/디스커버리_설계.md), [아키텍처 설계](../docs/PQC플랫폼_아키텍처_및_OSS경계_설계.md), [contracts](../contracts/).
> **이 문서가 다루는 것**: 읽기전용 인벤토리 뷰와 머신 메타데이터 스토어 — 관측을 모아 노드·앱에
> 이어 붙이고, 이력과 변화를 보여주는 부분.
...
인벤토리가 관리하는 머신 정보는 **기술 식별(node_id·지문)과 분리**된 두 종류를 가진다 — 식별은 기계가 관측(디스커버리 §4A), 이 둘은 **선언(CMDB)/리뷰어가 채우고 사용자가 수정**한다:

- **`MachineEndpoint`**(node_id·name·ip·port) — discovery 재접속용 **재사용 연결 메타데이터**. 사용자 hosts 파일에서 안전 부분집합만 적재(§4A.3). **접근 비밀은 미적재**(타입에 비밀 필드 없음).
...
**앱 귀속(§4A.2 자산 모델)** — 각 파생 `Finding`은 `app_keys`(복수)로 애플리케이션에 귀속되고 뷰에 `@app`으로 표시된다. host-wide 스캔에서 하나의 공유 라이브러리(예 `libcrypto.so.3`)를 여러 앱이 로드하면 **다중 귀속**된다 — 그 .so 교체의 영향 반경을 온전히 보이기 위함. 판단은 하지 않는다(§2.1).
...
type FinalizedPlan struct {
    ...
    DerivedFrom     string                 // 어떤 리컨실리에이션 스냅샷에서 (§0.2)
}
type PlanItem struct {
    NodeID          string
    RemediationClass string                // taxonomy 분기 (§4.3/§4.4)
    DeployAutomationLevel DeployAutomationLevel // L1/L2/L3 — 리뷰어 자산별 판정(§4.5)
    ProviderChoice  string                 // FIPS 라우팅 결과(§4.10): BC-FJA/내부/…
}
```
...
- **상태 어휘**(뷰 렌더링에 필요한 스키마 의미): `CONFIRMED` = 선언 ∩ 관측, `UNDECLARED` = 관측 only(**shadow 통신**), `UNOBSERVED` = 선언 only. `UNOBSERVED`가 "실제 없음"인지 "완전성 맵상 갭(원리상 못 봄)"인지는 Discovery **완전성 맵**(§2.7 갭≠부재)으로 갈린다 — 갭이면 재수집, 아니면 사람 판정.
...
| 결정·계획 히스토리 | provenance 판단·의도 계열 | §0.3 |
...
- **노드** = 스코프 마스터 등재 머신(사용자 IP). 미등재 관측 상대는 "등재 판정 요청"(§0.4/§5)로 별도 표시.
...
### 12.2 정직성 규정 (이 그래프의 신뢰성)

**"관측된 것"과 "원리상 못 본 것"을 구분한다**(§2.7, §3.3):
...
### 12.4 한계 (Discovery §2.5 network-collector에서 상속)

수동 관측 창·coverage 의존·암호화 한계 → **지도는 "본 것"이지 "존재하는 것 전체"가 아니다**. 이 부분성을 갭으로 정직히 표기하는 게 §0.2 감사 무결성과 일관된다. "가능한 한도내에서"가 정확한 프레이밍.

---

## 7. 이력·보존 (2026-07-21)

### 13.1 이력을 두는 목적은 셋이고, 요구가 서로 다르다

| 목적 | 필요한 것 | 반복 측정의 가치 |
|---|---|---|
| **변화 추적** | 바뀐 지점의 전/후 | 없음 — 같은 값 반복은 잡음 |
| **관측 증명** (감사에 "매일 스캔했다"를 보이기) | 언제·몇 번 봤는지 | **큼** — 이것이 증거 |
| **재계산 재현**(§0.2) | 원본 + 그때의 ruleset | ruleset 경계에서만 |

이 셋을 한 테이블이 떠안으면 무한 증식한다. 하루 1회 × 1000노드 × 3년 = 스냅샷 100만 건인데, 그중 의미 있는 변화는 노드당 연 몇 건뿐이다. **그래서 해법은 "지우기"가 아니라 "분리"다.**

### 13.2 2층 분리 — 무거운 것과 가벼운 것
...
무거운 저장은 **변화 횟수만큼만** 자라면서 "매번 봤다"는 사실은 온전히 남는다. §0.2도 깨지 않는다 — 원본을 고치는 게 아니라 **중복 저장을 안 하는** 것뿐이다.

이력 뷰(`-history`)는 이제 "변화 지점 + 그 상태를 몇 번·언제까지 재확인했는지(obs·observed)"를 보인다.

### 13.3 동등성 정의 — 이 설계의 급소 (`pkg/discovery/history/fingerprint.go`)
...
| JCA 축: jdk, **provider_set(순서 유지)** | 순서가 우선순위 협상을 결정한다(§1.2 (d)) — **정렬하면 안 된다** |
| Completeness: layers_missing, note | 갭이 달라지면 해석이 달라진다 |

> ⚠️ 여기서 빠뜨린 필드가 바뀌면 "변화 없음"으로 접혀 **이력에서 사라진다**. 계약에 필드를 더할 때 이 함수를 함께 갱신해야 한다.

`ExcludedByScope`(§14)는 지문에서 **뺀다** — 관리 대상 밖 자산의 증감은 관리 인벤토리의 변화가 아니다.

### 13.4 절단 정책 (`pqcota-prune`)

13.2로 같은 상태의 반복은 이미 접히므로, 절단이 다루는 것은 **오래된 변화 지점**뿐이다. 저장된 스냅샷은 전부 변화 지점이라 "변화점 보존" 같은 축은 필요 없다.

**불변식 셋:**

1. **수정 금지** — 남은 스냅샷은 바이트 그대로다. 절단은 과거를 *바꾸는* 게 아니라 보관을 *끝내는* 것이라 append-only(§0.2)와 양립한다.
2. **최신 불가침** — 노드별 최신은 어떤 정책으로도 지우지 않는다. 인벤토리 뷰와 프로비저닝 before 캡처(§8)의 근거이기 때문.
3. **지운 사실을 남긴다** — `pqcota_retention_events`에 기록하고 `-history`가 `⌫` 줄로 고지한다. 없으면 이력의 구멍이 **"관측을 안 함"과 구분되지 않는다** — §2.7 "갭 ≠ 부재"를 시간축으로 옮긴 것이다.
...
### 14.1 노드 게이트(§0.4)를 자산 단위로

노드를 등재해도 그 안에서 관측되는 것 **전부가 관리 대상은 아니다**. 시스템 기본 라이브러리, 패키지 매니저가 딸려 넣은 런타임, 일회성 프로세스가 섞이면 인벤토리가 잡음에 묻혀 못 쓰게 된다. 무엇을 계속 볼지는 **사용자가 선언**하고 도구는 그 선언을 집행한다(§0.1 — 판단은 마에스트로, 집행은 단원).
...
- **공유 `.so`는 귀속 앱이 여럿**이라(§4A.2) 그중 **하나만 맞아도** 규칙이 걸린다.

> **★ 공유 `.so` 제외의 영향 반경(footgun).** 공유 라이브러리는 귀속 앱이 여럿이라, 그중 **한 앱(예: 테스트 앱 `internal-test-*`)만 겨냥해 exclude**해도 그 .so를 함께 쓰는 **운영 앱(`payment-gw`)의 자산까지 함께 빠진다** — 하나만 맞아도 finding 전체가 걸리기 때문. 운영 앱을 지키려면 그 앱을 되살리는 **include를 exclude 뒤에** 둔다(뒤가 이긴다). 제외분은 §14.3대로 세어 고지되므로 "0건"으로 숨지는 않지만, **어떤 공유 자산이 어느 앱 때문에** 빠졌는지가 의도한 결과인지는 규칙 작성자가 확인해야 한다(테스트 TV-SCOPE-7).

### 14.3 ★ 제외 ≠ 부재

정책으로 뺀 자산을 조용히 사라지게 하면 인벤토리는 **"그런 게 없다"고 거짓말한다** — §2.7이 금지하는 바로 그것이다. 그래서 제외분을 세어 `Snapshot.ExcludedByScope`에 남기고, **적재 요약과 인벤토리 뷰 양쪽이 건수를 고지**한다.
