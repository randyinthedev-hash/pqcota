...
**근거**: 소스가 있으면 CI가 이미 스캔하거나 해야 할 일이라, pqcota가 CBOMkit을 오케스트레이션할 이유가 없다. 대체 불가 차별점은 전부 런타임 레인에 있다(§2.2 "JVM 공백", §2.4 "소스 부재=지배 케이스"). 소스 스캔은 이미 상품화된 영역이라 위임한다.
...
소스·아티팩트는 **pqcota가 스캔하지 않고** CBOMkit 등에 위임, 결과(표준 CycloneDX)만 받는다(§0.1 CI 위임). 상세 설계·라이선스·provenance 경계: **[위임수신_설계(이 문서)**.
...
- **결과**: 관측 계열(소스/아티팩트) 확보. *collector를 만들지 않는 대표 케이스(§0.1 위임).*
...
**책임**: 미리 생성된 CycloneDX(사용자 CI의 CBOMkit, 에어갭 오프라인 산출)를 **수신·처리**. **CBOMkit을 실행하지 않는다**(§0.1).
