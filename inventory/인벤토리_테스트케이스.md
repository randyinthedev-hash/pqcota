...
[인벤토리 설계](인벤토리_설계.md)(규정서 §3)를 **검증 가능한 인수 기준**으로 옮긴 것이다. 구현은 이 테스트를 통과하는 것을 목표로 한다(TDD).
...
적재→조회→이력→절단→스코프 **종단**은 여기 케이스가 아니라 [데모 5/6](../demo/통합_검증.md)이 확인한다.
...
절 제목의 `SV-*`는 설계 문서가 매긴 **상황**(Scenario·inVentory) 번호이고([인벤토리 설계](인벤토리_설계.md)의 선언 임포터, [위임 수신 설계](위임수신_설계.md)), 표 안의 `TV-*`는 그 상황을 검증하는 **테스트** 번호다.
...
| [TV-INGEST-2](../pkg/inventory/ingest/central_test.go) | `TestIngestSignatureReject` — 서명 검증 실패 | **거부**(§2.7) | 손댄 결과가 인벤토리에 들어오지 않게 한다 |
...
### TV-HISTORY. 이력 열람·변화 diff (설계 §13)
| 케이스 | Given → When | Then | 목적 |
|---|---|---|---|
...
| [TV-HISTORY-6](../pkg/inventory/render_test.go) | `TestRenderDiffWarnsOnRulesetChange` — `ruleset`이 다른/같은 두 스냅샷 diff | 다르면 재계산 경고, 같으면 안 뜸(§0.2) | 파생값 차이를 실제 변화로 읽으면 없던 변경을 쫓게 된다. 매번 뜨는 경고는 읽히지 않는다 |

### TV-RETENTION. 보존 정책 (설계 §13.2·§13.4)
| 케이스 | Given → When | Then | 목적 |
|---|---|---|---|
...
| [TV-SCOPE-7](../pkg/inventory/ingest/central_test.go) | `TestIngestReportsScopeExclusions` — 정책이 자산을 뺀 적재 | 적재 요약·스냅샷·인벤토리 뷰 **셋 다** 건수 고지, 제외한 자산은 남지 않음 | 스코프가 조용히 자산을 지우면 인벤토리가 거짓말을 한다(§2.7·§14.3) |
