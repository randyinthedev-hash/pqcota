...
→ node_id 같은 기술 ID로는 사람이 못 알아보므로, 표시명·환경 등을 별개 레인으로 둔다. 재사용·수정 가능, 비밀은 절대 저장 안 함.

**구현 — 런타임 서비스**:
- **`inventory.MetaStore`**(Mem/Pg) — 엔드포인트·프로필을 node_id PK로 **upsert**한다(히스토리 append-only와 대비 — 이 둘은 사용자가 재사용·수정하는 가변 메타데이터). Pg 스키마에 접근 비밀 컬럼이 없다.
...
**앱 귀속(§1.5 자산 모델)** — 각 파생 `Finding`은 `app_keys`(복수)로 애플리케이션에 귀속되고 뷰에 `@app`으로 표시된다. host-wide 스캔에서 하나의 공유 라이브러리(예 `libcrypto.so.3`)를 여러 앱이 로드하면 **다중 귀속**된다 — 그 .so 교체의 영향 반경을 온전히 보이기 위함. 판단은 하지 않는다(§2.1).
...
```
> `DeployAutomationLevel`·`RemediationClass`는 이미 contracts 통제 어휘. **여기서 채워진다**(Discovery 아님, MANUAL 리뷰어). `Decision`·`FinalizedPlan` 스키마는 contracts에 있다(공개 스키마).
...
- **노드** = 스코프 마스터 등재 머신(사용자 IP). 미등재 관측 상대는 "등재 판정 요청"(§1.4/§5)로 별도 표시.
...
- **미관측 엣지는 "연결 없음"으로 그리지 않는다** — 점선/⚪로 "unobserved" 표기. 완전성 맵과 연동.
- **coverage 표시**: collector 미설치 노드는 회색 처리(그 노드의 엣지는 반쪽만 보임).
- **capability vs actual 오버레이**: 노드에 "PQC 가능 lib 로드"(디스커버리 Finding)와 엣지 "실제 고전 협상"(관측)을 겹쳐 표시 → *"PQC 되는데 고전으로 폴백 중"* 같은 **정확한 다음 액션**이 드러남.
...
수동 관측 구간·coverage 의존·암호화 한계 → **지도는 "본 것"이지 "존재하는 것 전체"가 아니다**. 이 부분성을 갭으로 정직히 표기하는 게 §1.2 감사 무결성과 일관된다. "가능한 한도내에서"가 정확한 프레이밍.
...
| 목적 | 필요한 것 | 반복 측정의 가치 |
|---|---|---|
| **변화 추적** | 바뀐 지점의 전/후 | 없음 — 같은 값 반복은 잡음 |
| **관측 증명** (감사에 "매일 스캔했다"를 보이기) | 언제·몇 번 봤는지 | **큼** — 이것이 증거 |
...
- **스냅샷**(무거움, `pqcota_snapshots`) — 실질 내용이 **바뀔 때만** 쌓인다. 변화 추적·재계산 재현의 근거.
- **관측 기록**(가벼움, `pqcota_observations`) — 적재할 때마다 1행(`node_id, snapshot_id, ruleset, observed_at`). 관측 증명의 근거.

무거운 저장은 **변화 횟수만큼만** 자라면서 "매번 봤다"는 사실은 온전히 남는다. §1.2도 깨지 않는다 — 원본을 고치는 게 아니라 **중복 저장을 안 하는** 것뿐이다.
...
2. **최신 불가침** — 노드별 최신은 어떤 정책으로도 지우지 않는다. 인벤토리 뷰와 프로비저닝 before 캡처(§8)의 근거이기 때문.
3. **지운 사실을 남긴다** — `pqcota_retention_events`에 기록하고 `-history`가 `⌫` 줄로 고지한다. 없으면 이력의 구멍이 **"관측을 안 함"과 구분되지 않는다** — §2.6 "갭 ≠ 부재"를 시간축으로 옮긴 것이다.

**정책 판정**: `Policy{OlderThan, KeepLast}`. 두 축을 다 주면 **보수적으로** — 둘 다 "버려도 된다"고 할 때만 지운다(최근 N개 안이거나 아직 안 오래됐으면 보존). 파괴적 동작이라 의심스러우면 남긴다. 축을 하나도 안 주면 "최신만 남기고 전부 삭제"가 되므로 `ErrNoPolicy`로 **거부**한다.

**왜 별도 커맨드인가**: `Pruner`를 조회용 `Store` 인터페이스에서 분리하고 `pqcota-prune`을 따로 뒀다. 읽기 도구가 파괴적 동작을 겸하면 실수 한 번이 이력을 지운다. 기본은 dry-run이고 실제 삭제는 `-apply`로만.
...
노드를 등재해도 그 안에서 관측되는 것 **전부가 관리 대상은 아니다**. 시스템 기본 라이브러리, 패키지 매니저가 딸려 넣은 런타임, 일회성 프로세스가 섞이면 인벤토리가 잡음에 묻혀 못 쓰게 된다. 무엇을 계속 볼지는 **사용자가 선언**하고 도구는 그 선언을 집행한다(§1.1 — 판단은 마에스트로, 집행은 단원).

구현: `pkg/kernel/scope/asset.go`의 `AssetPolicy`. 적용 지점은 **정규화 직후·적재 직전**(`normalize.Normalize`) — 관측은 하되 관리 대상만 영속한다.
...
- 규칙이 없으면 전부 관리 대상 — 스코프를 안 쓰는 사용자를 막지 않는다.
- 규칙 축: `runtime`, `lib`(openssl은 soname, jca는 provider), `app_key`. 빈 칸·`*`는 모두, 패턴은 glob.
- **공유 `.so`는 귀속 앱이 여럿**이라(§1.5) 그중 **하나만 맞아도** 규칙이 걸린다.

> **★ 공유 `.so` 제외의 영향 반경(footgun).** 공유 라이브러리는 귀속 앱이 여럿이라, 그중 **한 앱(예: 테스트 앱 `internal-test-*`)만 겨냥해 exclude**해도 그 .so를 함께 쓰는 **운영 앱(`payment-gw`)의 자산까지 함께 빠진다** — 하나만 맞아도 finding 전체가 걸리기 때문. 운영 앱을 지키려면 그 앱을 되살리는 **include를 exclude 뒤에** 둔다(뒤가 이긴다). 제외분은 §8.3대로 세어 고지되므로 "0건"으로 숨지는 않지만, **어떤 공유 자산이 어느 앱 때문에** 빠졌는지가 의도한 결과인지는 규칙 작성자가 확인해야 한다(테스트 TV-SCOPE-7).
...
같은 이유로 이력 diff에서 스코프 적용은 "사라짐"으로 잡히는데, 이는 **자산이 없어진 게 아니라 관리 대상에서 뺐다는 뜻**이다(데모가 이 구분을 캡션으로 명시한다).
