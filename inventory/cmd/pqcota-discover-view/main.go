...
func main() {
	...
	for _, res := range results {
		...
		snap, err := normalize.Normalize([]*discoveryv1.CollectionResult{res}, "snap", node, normalize.RulesetVersion, history.NewMemStore(), nil)
		if err != nil {
...
// 알리지 않고 건너뛰었기 때문이다. 관측이 화면까지 오지 않으면 적지 않은 것과 같다.
//
