...
**새 개념에 이름을 붙일 때 영어를 먼저 떠올리지 않는다.** 문장 구조는 한 번 고치면 되는데
어휘는 그렇지 않다 — 기능을 붙일 때마다 새 개념에 이름을 달고, 그때 영어를 옮기면 문서가
다시 어색해진다. 낱말을 나중에 치환해도 다음 기능에서 또 생기므로 **이름을 정하는 그 자리에서**
잡아야 한다. 아래는 v0.2.0–v0.4.0에서 실제로 들어왔다가 되돌린 것들이다.

| 이렇게 부르지 않는다 | 이렇게 | 원문 · 왜 |
|---|---|---|
| 관측 **창** | 관측 **구간** | observation window — `창`은 화면의 window와 헷갈린다 |
| **실기**에서 확인했다 | **실제 장비**에서 확인했다 | 일본어 `実機` — 한국 개발자는 쓰지 않는다 |
| note가 **말한다** | note에 **적혀 있다** | the note says — 무생물은 말하지 않는다 |
| 부모 **사슬** | 부모 **체인** | parent chain — `사슬`은 쇠사슬이지 연결 구조가 아니다 |

### "안 된다"고 적었으면 왜인지를 그 자리에 붙인다

이 리포는 한계를 감추지 않으므로 부정형 문장이 많다 — *늘 되지는 않는다 · 확정하지 않는다 ·
만들지 않는다*. 그런데 **이유가 두 문장 뒤에 오면 읽는 사람은 그 사이를 추측으로 메운다.**
판단과 근거는 붙어 있어야 한다.
