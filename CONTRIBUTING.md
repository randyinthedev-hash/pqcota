...
**순수 함수로 테스트 가능하게.** 파싱·판정 로직을 I/O에서 떼어 실물(프로세스·DB·네트워크) 없이 단위 테스트되게 쓴다. 예를 들어 `ParseProcMaps(reader)`는 `/proc` 없이 돈다. **테스트는 동작만이 아니라 "왜 이 불변식인지"를 못박는다**(회귀 테스트엔 그 버그의 본질을 주석으로).
