...
**`pqcota-provision` refuses when there is no key to check with.** It used to warn and go on, and closing that path happened only where a deployment had set `PQCOTA_REQUIRE_APPROVAL=1`. That leaves approval integrity as something you *can* close while the default path stays open. The one way to open it knowingly is `--allow-unverified-approvals`, and **it has to be written on the command line** — if an environment variable could open it too, what was verified would hide in shell configuration, and the log alone would not say whether the artifact stands on a checked approval.
