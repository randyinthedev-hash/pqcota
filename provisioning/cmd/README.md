...
알고 여는 방법은 `--allow-unverified-approvals` 하나다. **명령줄에 적어야 열린다.** 환경변수로도 열리면 무엇이 검증됐는지가 셸 설정에 숨어, 로그만 보고는 이 산출물이 확인된 승인 위에 섰는지 알 수 없다. 열어도 「확인하지 않았다」는 그대로 나온다. 넘기는 것이지 확인한 것이 아니다.
