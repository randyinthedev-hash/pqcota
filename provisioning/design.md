...
검증 결과는 셋으로 갈린다. **확인됨**, **거부**(서명 꼴인데 키가 없거나 서명이 안 맞음), **확인 불가**(서명 꼴이 아닌 이름표). 통과·거부 둘로만 가르면 확인하지 않은 것이 확인한 것처럼 보인다(§2.6). 거부가 하나라도 있으면 생성을 멈추고, 확인된 승인이 하나도 없어도 멈춘다. 키가 아예 없으면 막지 않되 **확인하지 않았다고 크게 말하며**, 그 경로를 닫아야 하면 `PQCOTA_REQUIRE_APPROVAL=1`로 막는다.
