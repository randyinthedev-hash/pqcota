...
**확인할 키가 없으면 `pqcota-provision`이 거절한다.** 전에는 경고하고 통과시켰고, 닫는 것은 `PQCOTA_REQUIRE_APPROVAL=1`을 따로 건 배포에서만 일어났다. 그러면 승인 무결성이 「닫을 수 있는 수단」에 머물고 기본 경로는 열린 채다. 알고 여는 방법은 `--allow-unverified-approvals` 하나이고 **명령줄에 적어야 열린다.** 환경변수로도 열리면 무엇이 검증됐는지가 셸 설정에 숨어, 로그만 보고는 이 산출물이 확인된 승인 위에 섰는지 알 수 없다.
