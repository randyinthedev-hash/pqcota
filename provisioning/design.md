...
| 5. 단계적 배포 L1/L2/L3 | §4.3 단계적 배포 모델 |
| 6. 폐루프 | §4.3 (조치 후 재관측) |
| 6A. 히스토리·롤백 | §1.3 Provenance Chain |
| 9. GPL·서명 경계 | (라이선스는 [라이선스 정리](../docs/licensing.md)) |
...
| 배포 수준 | L1 stage / L2 install / L3 activate | 단계 경계 = 롤백 지점 (§4.3) |
| 환경 | 표준 / 에어갭 / **규제(FIPS)** | 배포 채널·provider 라우팅 |
...
`Executable(plan) error`는 확정 계획이 실행 근거로 유효한지 검증하는 **공유 계약 규칙**이다. 두 층을 본다. 앞 셋은 **절차**(계획이 어떻게 만들어졌나), 뒤 둘은 **내용**(그 계획으로 무엇을 할 수 있나)이다:
- `status == FINALIZED` (아니면 거부)
- `approval_signatures` 최소 1건(§3.3③ finalize 전제)
- `actions` 최소 1건
- 조치마다 `target_node_id`가 있을 것 — 없으면 플레이북 `hosts`에 빈 항목이 들어가 **어디에도 닿지 않는 play**가 나오는데, 그것은 실패가 아니라 아무 일도 없음으로 보인다
- 조치마다 `kind`가 정해져 있을 것 — `UNSPECIFIED`면 생성기가 분기하지 못해 「config로는 넣을 수 없는 조치」라고 적은 조각을 내는데, 그것은 사실이 아니라 **계획이 말하지 않은 것**이다(§2.5 — 추측하지 않는다)

**승인 서명의 값은 `Executable`이 보지 않는다.** 개수만 센다. 확인하려면 키가 있어야 하는데 이 함수는 계약 규칙이라 키 재료를 들지 않기 때문이다. 확인은 `sign.VerifyApprovals`가 하고 `pqcota-provision`이 `PQCOTA_APPROVAL_KEYS`로 배선한다. **게이트를 지났다고 승인이 확인된 것은 아니다** — 둘은 다른 질문이다.

승인 서명의 꼴은 `"<승인자>:ed25519:<base64>"`이고, 서명 대상은 승인 서명 자신을 뺀 계획 전부다(`sign.CanonicalPlan`). 승인자 id를 서명 문자열 **안에** 두는 이유는, 검증할 때 그 사람의 키로만 확인하기 위해서다. 키를 목록으로 받으면 어느 키로든 통과한 서명이 아무 이름이나 달고 들어와, 서명이 「누군가 승인했다」까지만 답하게 된다. 승인은 책임의 소재라 그 답으로는 부족하다. 그래서 `PQCOTA_APPROVAL_KEYS`는 처음부터 `<승인자>=<공개키>` 묶음이다.

검증 결과는 셋으로 갈린다. **확인됨**, **거부**(서명 꼴인데 키가 없거나 서명이 안 맞음), **확인 불가**(서명 꼴이 아닌 이름표). 통과·거부 둘로만 가르면 확인하지 않은 것이 확인한 것처럼 보인다(§2.6). 거부가 하나라도 있으면 생성을 멈추고, 확인된 승인이 하나도 없어도 멈춘다. 키가 아예 없으면 **거절한다**(아래).

**확인할 키가 없으면 `pqcota-provision`이 거절한다.** 전에는 경고하고 통과시켰고, 닫는 것은 `PQCOTA_REQUIRE_APPROVAL=1`을 따로 건 배포에서만 일어났다. 그러면 승인 무결성이 「닫을 수 있는 수단」에 머물고 기본 경로는 열린 채다. 알고 여는 길은 `--allow-unverified-approvals` 하나이고 **명령줄에 적어야 열린다** — 환경변수로도 열리면 무엇이 검증됐는지가 셸 설정에 숨어, 로그만 보고는 이 산출물이 확인된 승인 위에 섰는지 알 수 없다.

승인 서명은 `pqcota-approve`가 붙이고 키쌍은 `pqcota-keygen`이 낸다. **서명한 뒤에 계획을 고치면 승인은 무효가 된다** — 승인자는 계획의 이름이 아니라 조치의 내용에 책임을 지기 때문이다. `FillPlan`으로 조각을 채울 것이면 채운 뒤에 승인받는다.
...
| 들어온 상태 | 하는 일 |
|---|---|
| `IN_REVIEW` | 기존 승인과 `finalized_at`이 **없어야 한다.** 있으면 손상으로 거부한다 — 누군가 이 상태에 서명을 달아 둔 것이고, 그 서명은 이 상태에 대한 것이다. 조치가 1건 이상 있고 조치마다 대상 노드와 종류가 있어야 한다(`Executable`의 내용 층과 같다). 지나면 상태를 `FINALIZED`로, `finalized_at`을 지금으로 **먼저** 정한 뒤 그 상태를 서명한다 |
| `FINALIZED` | 기존 승인과 `finalized_at`을 **모두 갖춰야 한다.** 하나라도 없으면 손상으로 거부한다 — `FINALIZED`는 승인이 찍힌 뒤에만 생기는 상태라, 비어 있으면 상태만 바꿔 넣은 것이다. **구조 검사도 같이 지난다** — 첫 승인 뒤에 조치가 지워진 계획에 추가 승인이 붙으면 안 된다. 갖췄으면 아무것도 바꾸지 않고 서명만 더한다 |
| `DRAFT` · `UNSPECIFIED` | 거부한다. 승인할 대상이 아니다 |

**순서가 요점이다.** `CanonicalPlan`이 `status`와 `finalized_at`을 덮으므로, 서명 뒤에 바꾸면 방금 만든 서명이 깨진다. 두 번째 승인이 시각을 다시 찍어도 첫 서명이 깨진다 — 그래서 `FINALIZED`에서는 아무것도 바꾸지 않는다. 두 승인자가 **같은 정준 바이트**에 서명해야 둘 다 검증된다.

**승인 전에 구조를 묻는 이유**는, 여기서 걸리는 계획에 서명이 붙으면 「승인은 됐는데 실행할 수 없는 계획」이 생기기 때문이다. 그것은 승인자가 책임진 것이 무엇인지 말할 수 없는 상태다. 순서는 **불변식 → 구조 → 전이**이고, 거부된 계획은 상태·시각·승인 목록이 들어온 그대로다.

전에는 `pqcota-approve`가 상태를 아예 보지 않았다. `DRAFT` 계획에도 서명이 찍혔다. 뒤에서 `Executable`이 막아 악용되지는 않았지만, 승인이라는 행위가 무엇에 대한 것인지 확인하지 않는 자리였다. 그리고 판정한 쪽이 승인 칸에 검증되지 않는 이름표를 넣어 두면, 그 계획은 `FINALIZED`에 승인 항목이 있는 모양이라 **구조 관문의 의미를 충족한 것처럼 보였다.** 이제 그 모양은 손상으로 거부된다.

### 3.2 조치는 어느 스냅샷 상태에서 나왔나 — 참조와 해석 순서

`TraceabilityWarnings`가 알리던 「계획이 어느 스냅샷에서 나왔는지 모른다」를 **조치마다** 답할 수 있게 했다. 계획은 여러 노드에 걸치므로 계획 필드 하나로는 가리킬 수 없고, 계획을 만드는 쪽은 이력의 스냅샷 id를 재현할 수 없다(`ingest-<시각>:<노드>`). 그래서 참조는 **근거 단위**이고 **내용 지문**일 수 있다.
...
**복수 단위가 「스냅샷」이 아니라 「근거」인 이유.** 같은 자산을 원천 노드 여럿이 봤을 때 스냅샷만 여럿 두면 `finding_id`가 하나라 둘째 스냅샷의 어느 finding이 근거인지 말할 수 없다. 근거 하나가 finding과 그 스냅샷을 짝으로 든다. 주 근거가 앞이고, 호환용 `finding_id`는 `evidence_sources[0].finding_id`와 같아야 한다.
...
6. **못 찾으면** 불완전으로 세고(종료 3, `--allow-incomplete`로만 0) 경고문이 가능한 이유를 값으로 말한다 — 다른 이력, 원천 노드 이름, 규칙 판, 스코프 정책(제외 수), 결과 집합.
7. **찾았으면** `ProvisioningRecord.snapshot_resolutions`에 제출된 참조와 **실제 스냅샷 id**를 한 항목으로 짝지어 남긴다. `--dsn`이 없으면 찾지 않고 레코드도 만들지 않는다 — 찾지 않은 것이지 못 찾은 것이 아니라 불완전으로 세지 않는다.

**참조가 가리키는 것은 「관측 사건」이 아니라 「중복 억제된 스냅샷 상태」다.** 같은 상태를 다시 관측하면 이력은 새 행을 만들지 않는다.

**이 필드들이 `CanonicalPlan`에 들어가므로 기존 승인 서명은 전부 무효다.** 덮지 않으면 참조를 서명 뒤에 바꿔 넣을 수 있어 「되짚을 수 있다」가 보장이 아니다. 빈 목록도 정규형에 자리 하나를 차지하므로 근거가 없던 옛 계획도 다시 승인받아야 한다.

**되짚을 수 있는가는 막지 않는다.** `derived_from_snapshot_id`·`ruleset_version`·`finalized_at`·조치의 `finding_id`가 비어도 계획은 실행된다. 다만 실행하고 나면 이력에 근거가 남지 않으므로 `TraceabilityWarnings`가 표면화한다. 같은 이유로 `TargetAlgorithmWarnings`는 목표가 하이브리드 그룹으로 풀리지 않아 **배치해도 아무것도 켜지지 않는** 조치를 알린다. 차단과 경고를 가르는 기준은 하나다: 사람이 손으로 채우는 것이 정당한 경로면 경고, 도구가 거짓을 적게 되면 차단이다.
...
# ↑ 우선순위 2 — PQC 알고리즘이 먼저 디스패치되게(수용 원칙 §2.2(d) 순서 협상).
...
| 서명 provenance | **승인 서명**은 등록된 승인자 키와 대조하고 어긋나면 거절한다(`PQCOTA_APPROVAL_KEYS`). **collector 리포트 서명**은 아직 키 목록으로만 확인해 어느 키가 누구 것인지 묶지 않는다(`sign.VerifyFrom` 배선 보류) |
...
- **Ansible 플레이북 = 데이터**(코드 링크·번들 아님)라 GPL 전염과 무관 → 플레이북을 **생성**하는 것은 정당하다(§4.3·[라이선스 정리](../docs/licensing.md)).
- **BouncyCastle 표준판**(MIT계열)은 GPL 격리 대상이 아니다(번들 가능). **BC-FJA(FIPS)**는 별도 계약 조건을 확인한다(§4.4).
- **서명 provenance**: 형식·검증 함수는 이 리포(`pkg/kernel/sign`)에 있다. **승인 서명**은 등록된 승인자 키와 대조해 거절까지 한다. **collector 리포트 서명**은 키를 목록으로 받아 「누군가 서명했다」까지만 답한다 — collector에 묶으려면 환경변수 형식을 바꿔야 해서 `sign.VerifyFrom`의 배선이 보류다.
