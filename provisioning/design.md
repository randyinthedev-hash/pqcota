...
**확인할 키가 없으면 `pqcota-provision`이 거절한다.** 전에는 경고하고 통과시켰고, 닫는 것은 `PQCOTA_REQUIRE_APPROVAL=1`을 따로 건 배포에서만 일어났다. 그러면 승인 무결성이 「닫을 수 있는 수단」에 머물고 기본 경로는 열린 채다. 승인 검증을 생략하려면 `--allow-unverified-approvals`를 **명령줄에 적어야 하고, 다른 수단은 없다.** 환경변수로도 열리면 무엇이 검증됐는지가 셸 설정에 숨어, 로그만 보고는 이 산출물이 확인된 승인 위에 섰는지 알 수 없다.
...
6. **못 찾으면** 불완전으로 세고(종료 3, `--allow-incomplete`로만 0) 경고문이 가능한 이유를 값으로 적는다. 다른 이력, 원천 노드 이름, 규칙 판, 스코프 정책(제외 수), 결과 집합이다.
...
**되짚을 수 없는 계획도 막지는 않는다.** `derived_from_snapshot_id`·`ruleset_version`·`finalized_at`·조치의 `finding_id`가 비어도 계획은 실행된다. 다만 실행하고 나면 이력에 근거가 남지 않으므로 `TraceabilityWarnings`가 그 공백을 드러낸다. 같은 이유로 `TargetAlgorithmWarnings`는 목표가 하이브리드 그룹으로 풀리지 않아 **배치해도 아무것도 켜지지 않는** 조치를 알린다. 차단과 경고를 가르는 기준은 하나다: 사람이 손으로 채우는 것이 정당한 경로면 경고, 도구가 거짓을 적게 되면 차단이다.

파생이 아니라 실행 직전 관문이다. 이 함수를 통과했다고 곧바로 실행되는 것은 아니며, "무엇이 실행 근거인가"라는 규칙만 정한다.
...
승인자가 서명한 위임 수준과 실제 실행 수준이 달라지는 자리였다. 위험도에 따라 위임을 나눈 판정이
