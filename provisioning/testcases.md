...
| [TP-GATE-18](../provisioning/cmd/pqcota-provision/main_test.go) | `TestUnknownLevelIsRefused`: `--level L3`·`l4`·`full` | **사용법 오류(2)로 끝나고 산출물이 없다** | 모르는 값을 조용히 L2로 삼키면 활성화·재시작이 빠진 산출물을 받고도 시킨 대로 됐다고 읽는다. 말한 것보다 낮게 도는 것도 잘못이다 |
...
| [TP-GATE-13](../pkg/provisioning/snapshot_test.go) | `TestReferenceShapeIsCheckedWithoutHistory` · (CLI) `TestMalformedSnapshotReferenceIsIncompleteWithoutDSN`: 원천 노드 없음 · 빈 id · 참조 종류 없음 · 내용 참조의 셋 중 빈 것 · 모르는 규격 판 · 16진수 64자 아님 · 대문자 / 멀쩡한 참조를 이력 없이 | 일곱은 **잘못된 참조**이고 불완전으로 센다. CLI 는 `--dsn` 없이 **종료 3**, 무엇이 틀렸는지 stderr 에. 멀쩡한 참조는 이력이 없으면 「찾지 않았다」이고 불완전이 아니다 | 틀린 참조는 이력이 있든 없든 틀린 것이다. DSN 이 있을 때만 알리면 로컬에서 만든 계획의 결함이 배포 직전에야 드러난다. 찾지 않은 것과 못 찾은 것은 다르다 |
| **[TP-GATE-14](../pkg/provisioning/snapshot_test.go)** | `TestResolveFindsAndChecksFindingMembership` · `TestResolveActionWalksEvidenceThenLegacy` · `TestTraceabilityPrefersActionEvidence`: 실제 id 와 내용 지문으로 찾기 / 찾은 스냅샷에 그 finding 이 없음 / 다른 노드의 스냅샷 id / 규칙 판이 다른 지문 / 근거 둘 가운데 하나만 찾힘 / 호환 `finding_id` 가 주 근거와 다름 / 근거 없이 계획 단위 id / 근거도 계획 단위 id 도 없음 | 양쪽으로 찾힌다. **finding 이 없으면 잘못 짝지어진 근거**로 미해결·불완전(양쪽 다). 다른 노드는 찾은 것이 아니다. 규칙 판 사유가 값으로 나온다. 결과가 근거 순서대로 짝지어지고 못 찾은 항목도 제출된 참조를 잃지 않는다. 호환 `finding_id` 가 **비어 있어도** 미해결이고, 근거 자체에 `finding_id` 가 없으면 참조가 맞아도 미해결이다. legacy 는 `SnapshotReference` 로 합성하지 않고 `legacy_plan_snapshot_id` 로 남는다. 둘 다 없으면 빈 목록이고 추적성 경고가 말한다 | 스냅샷 id·노드·지문만 맞고 finding 이 그 안에 없는 짝이 통과하면 참조는 맞는데 근거는 틀린 레코드가 남는다. 제출과 해결을 따로 반복하면 하나만 못 찾았을 때 어느 참조가 어느 id 로 해결됐는지 알 수 없다 |
