...
**케이스는 전부 unit이다**. 실물 없이 어디서나 돈다. 예외는 TP-RECORD-3 하나로, `PQCOTA_TEST_DSN`이 있으면 실 Postgres로도 돌고 없으면 스킵한다(**스킵은 통과가 아니다**). CI는 Postgres 서비스를 붙여 이 케이스를 늘 돌린다.
...
| [TP-GATE-13](../pkg/provisioning/snapshot_test.go) | `TestReferenceShapeIsCheckedWithoutHistory` · (CLI) `TestMalformedSnapshotReferenceIsIncompleteWithoutDSN`: 원천 노드 없음 · 빈 id · 참조 종류 없음 · 내용 참조의 셋 중 빈 것 · 모르는 규격 판 · 16진수 64자 아님 · 대문자 / 멀쩡한 참조를 이력 없이 | 일곱은 **잘못된 참조**이고 불완전으로 센다. CLI는 `--dsn` 없이 **종료 3**으로 끝나고, 무엇이 틀렸는지는 stderr에 나온다. 멀쩡한 참조는 이력이 없으면 「찾지 않았다」이고 불완전이 아니다 | 틀린 참조는 이력이 있든 없든 틀린 것이다. DSN이 있을 때만 알리면 로컬에서 만든 계획의 결함이 배포 직전에야 드러난다. 찾지 않은 것과 못 찾은 것은 다르다 |
| **[TP-GATE-14](../pkg/provisioning/snapshot_test.go)** | `TestResolveFindsAndChecksFindingMembership` · `TestResolveActionWalksEvidenceThenLegacy` · `TestTraceabilityPrefersActionEvidence`: 실제 id와 내용 지문으로 찾기 / 찾은 스냅샷에 그 finding이 없음 / 다른 노드의 스냅샷 id / 규칙 판이 다른 지문 / 근거 둘 가운데 하나만 찾힘 / 호환 `finding_id`가 주 근거와 다름 / 근거 없이 계획 단위 id / 근거도 계획 단위 id도 없음 | 양쪽으로 찾힌다. **finding이 없으면 잘못 짝지어진 근거**로 미해결·불완전(양쪽 다). 다른 노드는 찾은 것이 아니다. 규칙 판 사유가 값으로 나온다. 결과가 근거 순서대로 짝지어지고 못 찾은 항목도 제출된 참조를 잃지 않는다. 호환 `finding_id`가 **비어 있어도** 미해결이고, 근거 자체에 `finding_id`가 없으면 참조가 맞아도 미해결이다. legacy는 `SnapshotReference`로 합성하지 않고 `legacy_plan_snapshot_id`로 남는다. 둘 다 없으면 빈 목록이고 추적성 경고가 알린다 | 스냅샷 id·노드·지문만 맞고 finding이 그 안에 없는 짝이 통과하면 참조는 맞는데 근거는 틀린 레코드가 남는다. 제출과 해결을 따로 반복하면 하나만 못 찾았을 때 어느 참조가 어느 id로 해결됐는지 알 수 없다 |
...
| # | 대상 | 케이스 | 레벨 |
|---|---|---|---|
| 1 | **실행 게이트**(finalized-only) · 경고 표면화 · CLI 배선 | TP-GATE-1–8·15–19 | unit |
...
**핵심 인수 기준**은 **TP-GATE-1(finalized+서명+조치 아니면 실행 거부)**이다. 확정되지 않은 계획으로 머신을 건드리지 못하게 하는 최강 게이트다. 그리고 **TP-RENDER-4·TP-PLAYBOOK-2(config로 못 넣는 조치는 정직하게 주석·L2에서는 재시작을 만들지 않음)**. 단계 경계가 코드로 강제됨을 보장한다. **TP-PLAYBOOK-6–9**는 L3 훅의 의미 순서·롤백 대칭, 빈 훅에서 명령을 지어내지 않음, 그리고 **조각·재시작이 서로를 덮어쓰지 않음**을 못 박는다.
