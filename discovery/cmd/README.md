...
0인 이유는 이 갭이 중앙까지 가야 하기 때문이다. Ansible로 여러 노드를 돌릴 때 종료코드가 1이면 그 태스크가 실패로 처리돼 결과 파일을 회수하지 않고, 중앙에는 그 노드에 대한 기록이 **아무것도** 남지 않는다. 그러면 인벤토리 뷰에서 "이 노드엔 TLS 링크가 없다"로 읽히는데, 실제로는 관측하지 못한 것이다. 갭을 실어 보내야 "관측하지 못했다"와 "없다"가 구분된다.
