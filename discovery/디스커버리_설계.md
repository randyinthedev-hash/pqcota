...
**문서 성격**: Discovery 서브시스템의 기술 설계. 상황별 인수 기준은 [테스트케이스](디스커버리_테스트케이스.md)(SD-1–SD-7). 규정서 §2, [아키텍처 설계](../docs/PQC플랫폼_아키텍처_및_OSS경계_설계.md), [contracts SSOT](../contracts/)를 구현 레벨로 잇는다.
...
| 이 문서 | 규정서 |
|---|---|
| 1. 컴포넌트 아키텍처 | §2.1 목적과 경계 · §6.1 intake 계약 |
| 2. Collector 상세 설계 | §2.2 collector 지형 · §2.3 3계층 교차검증 · §2.4 소스 부재 |
| 3. 정규화 파이프라인 6단계 | §2.5 정규화 파이프라인 (원본 불변은 §0.2) |
| 4. 스코프 게이트 & 라우터 | §0.4 스코프 마스터 |
| 4A. 머신 식별·UID 체계 | §0.5 머신 식별·자산 계층·접근 경계 |
| 5. 완전성 맵 | §2.7 산출물/무결성 (갭≠부재는 §2.6) |
| 6. Collector 배포 | §4.5 단계적 배포 · §4.6 신뢰 경계 |

자동화 등급(AUTO/PROPOSE/MANUAL)은 §2.6이 정하고 이 문서는 그것을 전제로 만든다 — 등급 자체를
여기서 다시 정하지 않는다.
...
| 증거 가용성 | 소스 / 아티팩트 / 바이너리만 / 실행 중 | `evidence_strength` (§2.4) |
| 호스트 환경 | 베어메탈·VM / 컨테이너 / 에어갭·배치 | 배포·네임스페이스 (§2.4) |
| 배포 티어 | T1 self-service / T2 substrate / T3 상주 | 신뢰장벽 (§2.4) |
| 스코프 상태 | 등재 / 미등재-관측 | 게이트 (§0.4) |
| 수집 결과 | confirmed / 열화 / unknown / 갭 | 완전성 맵 (§2.7) |
...
```
  ...
  └───────────────────────────┬───────────────────────────────────────┘
                              ...
                              ▼
   [정규화 파이프라인 6단계]  §2.5 ── 강화 입력: crypto-registry(provider 시그니처)
   raw수집→파싱→강화→검증→동일성해소→영속화
     │                              └ evidence_strength·fork판별·역할태깅 (정직한 증거 계층)
     ├─▶ [완전성 맵]  갭≠부재  SD-6
     ├─▶ [스코프 밖 라우터]  등재판정요청  SD-5
     ▼
   [디스커버리 히스토리] append-only, 재현가능(§0.2)  ──▶ 읽기전용 인벤토리 뷰
```

**계약 하나**: `Collector` intake 계약(§6.1) 뒤에 무엇이 있든 코어는 모른다 — collector가 늘어도 코어는 그대로다. 결과가 어떻게 얻어진 것인지는 `Envelope.detection_method`로만 드러난다.
...
**책임**: OpenSSL 런타임의 파일·프로세스·심볼 계층 수집. `ldd`/`lsof`/`ss`/`readelf`에 **의존하지 않고 Go로 자립 구현**(최소 이미지 대응, §2.4 발자국).
...
**fork 시그니처 매처 (SD-3 핵심 IP)** — 동일 soname 문제(§1.2) 해결:
```go
...
// 매칭 실패 → OpensslAxes.Fork = "" (unknown, §2.6). evidence_strength=inferred-low.
```
**산출**: CycloneDX 컴포넌트(lib) + `pqcota:` properties(crypto_runtime=openssl, detection_method, openssl.fork, binding_mode) + raw_capture(네이티브 JSON) + Envelope. **Finding 파생은 코어**(§2.5 계약).
...
**정찰이 선행한다 — openssl(§2.1)과 대칭.** openssl의 `ScanHost`가 `/proc`를 훑어 로드된 lib를 스스로 찾듯, jvm도 `ScanJVMs`가 실행 중 JVM을 **직접 조사한다**(호출자가 PID·JDK를 미리 알아 넘기던 비대칭 제거). 접근 불가 프로세스는 `Denied`로 세어 완전성 갭의 원천으로(§2.6). `AttachAll`이 발견한 각 JVM에 attach하고, **attach 실패도 조용히 버리지 않고 갭으로** 센다(openssl의 프로세스별 탐지 합산과 대칭). 구현: `collectors/jvm/{procscan,attach}.go`.

**다중 JVM 식별 — 앱 단위, PID 아님.** 한 노드에 JVM이 여럿이면 각각 **구별되는 finding**이어야 한다(하나가 dedup으로 사라지면 §2.7 정직성 위반 — 실제 자산 은폐). 식별자는 **앱**(cmdline의 main 클래스·`-jar`) 우선, 없으면 JAVA_HOME. **PID는 쓰지 않는다** — 매 스캔 달라져 finding id가 흔들리고 이력이 "매번 새 자산"으로 깨진다(같은 JDK의 두 앱도 앱 키로 갈린다).
...
2. **보장 폴백**: attach 실패 시 **정적 경로**(java.security 등록 순서·JAR/classpath 시그니처 스캔)로
   ...
   **갭 기록**(§2.6 "unknown 1급", 자동 부재 금지). **동적 등록(addProvider)은 이 경우 사각지대**로
...
dynamic-trace(PROPOSE)보다 가볍다. 단 데이터 평면을 건드리므로 handshake-only 필터 + 자기참조 회피(§2.7).

**한계(반드시 갭으로 표기)**:
- **관측 창**: 캡처 중 흐른 트래픽만 — 유휴·배치·DR 링크 미관측 → 갭(≠부재, §2.7). 시간대별 반복(§2.3).
- **coverage 의존**: collector 도는 호스트의 연결만. 양쪽 다 미설치 엣지는 SPAN/탭 없으면 안 보임.
- **귀속**: 스코프 밖 IP·NAT·프록시 → "등재 판정 요청"(§5).

> **Phase 1 기능**(§7 로드맵 "관측 병행 + shadow 발견"). 이 엣지 관측이 인벤토리 reconciliation의
...
규정서 §2.5를 인터페이스로 고정. **강화·검증·동일성·영속화가 코어 단독**(§0.2 재계산 위해).
...
| ⑥영속화 | → 디스커버리 히스토리 append | `derived_from_snapshot_id`+`ruleset_version` | §0.2 |

강화의 유일 소스는 코어. Collector가 같은 로직을 중복 구현하지 않는다 → 규칙 개선 시 원본에서 재계산(§0.2).

---

## 4. 스코프 게이트 & 라우터 — SD-5

- **사전 게이트**: `CollectRequest.target_node_ids`는 코어가 스코프 마스터로 **이미 필터**한 것만(§0.4). Collector는 받은 것만 수집.
...
> 규칙은 **§0.5**가 정한다 — 신원·상관·위치 3층 분리, PID 미저장, 접근 비밀 미적재. 여기서는
> 그것을 무엇으로 구현하는지를 적는다.
...
| 층 | 무엇 | 비고 |
|---|---|---|
| **권위 ID** | `node_id` = 스코프 마스터(CMDB) (§0.4) | **주 경로: 사용자가 discovery 시작 시 입력**(Ansible inventory/CMDB). 안정·유일 |
...
- **자동 self-id (폴백)**: CMDB 없이 bare 실행 시 지문에서 **결정론적** 파생(`machineid.SelfAssign`: cloud>machine-id>hw>fqdn 우선순위 → `"node:"+sha256[:16]`). 같은 머신→같은 값이라 스캔마다 중복 안 생김. §0.4상 권위 아님 → RegistrationRequest.
- **사용자 입력 중복/충돌 검증**: 사용자 node_id는 오류 가능 → 지문으로 교차검증(`ingest.CheckIdentity`). 한 물리머신키→여러 node_id=**중복**(한 머신 여러 이름), 한 node_id→여러 키=**충돌**(한 이름 여러 머신·재이미지). 판정 안 하고 surface만(§2.6, 사람/reconcile 몫).
...
- **배치/간헐 노드**: 수집 시점 미실행 → **갭 기록, 자동 "부재" 금지**(§2.6). 시간대별 반복 수집(§2.3)으로 dlopen·배치 누락 방지.
- 이 구분이 Inventory의 UNOBSERVED 판정에서 "실제 없음"과 "원리상 못 봄"을 가르는 근거(§2.7).

## 6. Collector 배포 (호스트 도달) — 시나리오별

아키텍처 §2.3 원칙을 시나리오에 매핑. **이 리포는 collector CLI + 결과 서명 + 스코프 게이트만; 자체 push 엔진 없음**(§4.6).
...
**T1 가드레일(self-service)**: ① 최소 caps — root 아님, `CAP_NET_RAW`(network-collector)·`CAP_SYS_PTRACE`(/proc)만. ② 번들 digest 핀 + 서명 검증 + 멱등(포크 가능한 투명 아티팩트). 번들이 올바른 호출(caps·co-location·버전 핀·재시도)을 1회 인코딩. ③ 대상은 사용자 스코프 마스터(§0.4). 실행 주체가 pqcota가 되면 T3(상주 에이전트)가 되는데, 그건 만들지 않는다.
...
| **SD-4** 컨테이너 | 동일 collector + 네임스페이스 배포(§6) + 완전성 맵 | 주입 성공=confirmed / 실패=갭 |
| **SD-5** 스코프 밖 | 스코프 게이트·라우터(§4) | 등재판정요청(PROPOSE) |
| **SD-6** 배치 노드 | 완전성 맵(§5) + 시간대별 반복 | 갭 기록(≠부재) |
| **SD-7** 에어갭 | T1 오프라인 번들(§6) | T1 배치 수집 |
...
| **SD-3** 바이너리 | fork·version 시그니처 판별 | strings/readelf 원시만 | ★ **fork 시그니처 매처**(동일 soname 구분 §1.2) | **높음·IP** |
...
3. **정직한 증거 계층** — evidence_strength·완전성 맵(갭≠부재)·provider 레지스트리. *어떤 기존 도구에도 없고, §0.2 감사 무결성의 근거.*
