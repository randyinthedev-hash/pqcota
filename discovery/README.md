...
실행 중인 시스템이 **실제로 어떤 암호 알고리즘을 쓰는지** 관측한다 — 암호 자체(암호문·키)가 아니라 **어떤 라이브러리·provider·알고리즘이 로드·등록·협상되는지**다. 정적 문서·소스 스캔이 관측되지 않는 **런타임 실체**(동적 등록된 provider, 로드된 라이브러리, 협상된 통신 그룹)를 잡아, 각 자산에 양자내성 등급(🟢 PQC/하이브리드 · 🔴 고전=양자취약 · ⚪ 불명)를 부착한다.
