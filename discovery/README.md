...
실행 중인 시스템이 **실제로 어떤 암호 알고리즘을 쓰는지** 관측한다. 암호 자체(암호문·키)가 아니라 **어떤 라이브러리·provider·알고리즘이 로드·등록·협상되는지**다. 정적 문서·소스 스캔으로는 관측되지 않는 **런타임 실체**(동적 등록된 provider, 로드된 라이브러리, 협상된 통신 그룹)를 잡아, 각 자산에 양자내성 등급(🟢 PQC/하이브리드 · 🔴 고전=양자취약 · ⚪ 불명)을 부착한다.
...
| 자산이 자기 프로세스 것만 잡힌다 | root가 아니다. 다른 사용자의 `/proc`은 관측하지 못한다(관측하지 못한 만큼 갭으로 고지) |
...
| 관측 엣지가 0이다 | 관측 구간 동안 핸드셰이크가 흐르지 않았다. 유휴 링크는 실환경에서도 안 보인다. **부재가 아니라 미관측**이다 |

## 더 알아야 한다면

collector가 어떻게 파싱하고 어디까지 열화되는지, 정규화 6단계·완전성 맵·스코프 게이트가 무엇인지 → **[디스커버리 설계](design.md)**.
...
- **설계 문서**: [디스커버리 설계](design.md) · [collector 배포](collector-deployment.md) · [테스트케이스](testcases.md)

## 더 보기

규정서 §2 · [아키텍처 설계](../docs/architecture.md) · 정규화·히스토리 라이브러리 [`pkg/discovery/`](../pkg/discovery) · 실행 예제 [`examples/discovery/`](../examples/discovery)
