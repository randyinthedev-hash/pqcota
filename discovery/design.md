...
- **관측 레인과 선언 레인.** collector가 *본 것*(observed)과 사용자가 *신고한 것*(declared/CMDB)을 Envelope의 `detection_method`로 구분해 라벨한다. 그 라벨이 인벤토리에서 둘을 가르는 근거가 된다.
- **`evidence_strength`.** `confirmed`(실행 중 실체) → `inferred`(정적 추정) → `unknown`. 관측 방법에 따라 결정론적으로 부착된다.
- **완전성 맵(갭은 부재가 아니다).** 관측하지 못한 계층은 "없음"으로 단정하지 않고 **갭**으로 명시한다. 이 정직성이 감사 무결성·재현성의 근거다.
...
> **플랫폼 제약**: **전 바이너리가 Go**다(collector도 운영자 CLI도). 유일한 비-Go는 jvm-collector의 Java 사이드카뿐이다. 그래서 **OS 갈림은 언어가 아니라 "무엇을 만지느냐"로 정해진다**:
...
> - **같은 축을 OS마다 다른 API로 보는 것도 있다**. jvm(§2.2)의 정찰이 그렇다. 리눅스는 `/proc`, Windows는 Toolhelp32다. 이때 달라지는 것은 **커버의 깊이**이지 되고 안 되고가 아니다.
...
> **provider 층은 보지 않는다.** 여기까지가 lib 경로·fork·버전이고, 그 위에 얹힌 provider(예: `oqsprovider`가 이미 깔려 있는지)는 관측 대상이 아니다. jvm-collector가 attach로 provider 체인 전부를 보는 것(§2.2)과 다른 지점이다. 프로비저닝을 막지는 않는다: 어떤 provider를 쓸지는 계획에 사용자가 적고, 버전 근거는 인벤토리에 있다. 조치 뒤 재관측해도 이 층의 변화는 인벤토리에 나타나지 않는다.

**fork 시그니처 매처 (SD-3 핵심 IP).** 동일 soname 문제(수용 원칙 §2.2)를 푼다:
```go
...
3. **운영 옵션**: confirmed 증거가 필요한 고가치 자산은 배포 시 대상 JVM에
   ...
   플랫폼이 강제하지 않는다. 사용자 자산의 기동 방식은 사용자의 것이고, 이는 Deploy의 스크립트 경계(§6.1)와 대칭이다.
...
**기술적 근거는 복호화 없이 관측한다는 것**이다: 핸드셰이크의 알고리즘 협상은 **평문**이다.
- **TLS**: ClientHello `supported_groups`·`key_share`, ServerHello 선택 그룹 → KEX 그룹(X25519MLKEM768 등)·cipher·버전 관측.
- **SSH**: KEXINIT의 KEX 알고리즘(예 `sntrup761x25519` PQC 여부)을 본다. 레거시는 SSH 비중이 커 가치가 크다.
...
**어느 앱이 연 통신인가**(v0.3.0). 캡처 시점에 소켓 inode(`/proc/net/tcp`·`tcp6`)를 `/proc/*/fd`와
대조해 `ObservedEdge.app_key`를 채운다. 엣지가 노드까지만 가면 「이 서버 어딘가」에서 멈추는데, 사람이
조치할 대상은 서버가 아니라 앱이다. 실제 장비에서 재 보고 규칙 셋이 정해졌다.

- **한 소켓을 여러 프로세스가 쥔다.** fd는 상속되므로 부모가 연 연결을 자식들이 그대로 들고 있다.
  먼저 찾은 PID를 쓰면 연결을 연 쪽이 아니라 물려받은 쪽을 짚는다. 같은 inode를 쥔 PID 집합에서
  **부모 체인을 거슬러 가장 얕은 것**을 고른다.
- **늘 짚어내지는 못한다.** 핸드셰이크를 회선에서 본 순간과 `/proc`을 읽는 순간 사이에 소켓이 이미
  닫혀 있을 수 있다. 구현으로 줄일 수는 있어도 없앨 수 없는 경합이라 **best-effort다.** 그래서 앱을
  짚는 일은 캡처가 끝난 뒤 몰아 하지 않고 **엣지를 보는 그 자리에서** 한다.
...
자동 경로가 원리상 못 보는 자리는 사람이 선언으로 메운다 → [인벤토리 설계 §2](../inventory/design.md#2-데이터-모델).

**한계(반드시 갭으로 표기)**:
- **관측 구간**: 캡처 중 흐른 트래픽만 본다. 유휴·배치·DR 링크는 관측되지 않아 갭이 된다(부재가 아니다, §2.6). 시간대별 반복(§2.3).
...
- 사용자가 머신 접근 정보 파일(node_id·name·ip·port·계정·**키 또는 비밀번호**)을 **직접 관리**하고 **discovery 실행마다 지정**한다(`pkg/inventory.ParseHosts`, CSV 헤더 필수·순서 자유). 인증은 **호스트별로 독립**이다. `ssh_key`(개인키 경로, 권장) 또는 `ssh_pass`(비밀번호, 지원하나 권장 안 함; Ansible 접속엔 컨트롤러에 `sshpass` 필요). 키 생성·등록(`ssh-keygen`·`ssh-copy-id`)은 [examples/discovery](../examples/discovery/README.md) 참고.
- pqcota는 그 파일을 읽어: (a) **런타임 전용** Ansible 인벤토리 생성(`RenderAnsibleInventory`, 비밀 포함·미영속)으로 실행, (b) **안전 부분집합만 인벤토리 적재**(`MachineEndpoint`: node_id·name·ip·port이며 재사용·수정용이다).
- **접근 비밀(암호·SSH 키·계정)은 pqcota 인벤토리에 적재하지 않는다.** `MachineEndpoint` 타입에 비밀 필드가 아예 없어 컴파일 타임 보장. 비밀은 파일→Ansible 전달 후 버림.

## 5. 완전성 맵: SD-6 (갭은 부재가 아니다)
...
| 시나리오 | 배포(호스트 도달) | 실체 |
|---|---|---|
| SD-1·SD-2·SD-3 (일반) | **T1 self-service**다. 서명된 번들을 직접 실행하거나 자기 substrate로 감싸 돌린다 | 서명 번들 + 원커맨드 실행 |
...
- **JEP 451이 동적 agent 로딩을 제약한다**(대응 판정됨, §2.2 열화 경로): 향후 JDK가 `-XX:+EnableDynamicAgentLoading` 없이 기동된 JVM의 dynamic attach를 막을 수 있다. 이 리포는 정적 경로(java.security·JAR 스캔)로 열화 + 완전성 맵 갭 기록, **회복 탐지(비-agent 경로로 실체 조회)는 하지 않는다**. 남은 열린 질문: 비-agent 경로(JMX/로컬 JVMTI)로 `getProviders()` 실체를 어디까지 조회 가능한지(별도 collector 티어 상세).
...
3. **정직한 증거 계층**: evidence_strength·완전성 맵(갭은 부재가 아니다)·provider 레지스트리. *어떤 기존 도구에도 없고, §1.2 감사 무결성의 근거.*
