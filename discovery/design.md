...
- **완전성 맵 (갭 ≠ 부재)** — 못 본 계층은 "없음"으로 단정하지 않고 **갭**으로 명시한다. 이 정직성이 감사 무결성·재현성의 근거다.
...
**책임**: OpenSSL 런타임의 파일·프로세스·심볼 계층 수집. `ldd`/`lsof`/`ss`/`readelf`에 **의존하지 않고 Go로 자립 구현**(최소 이미지 대응, §2.3 발자국).
...
**한계(반드시 갭으로 표기)**:
- **관측 창**: 캡처 중 흐른 트래픽만 — 유휴·배치·DR 링크 미관측 → 갭(≠부재, §2.6). 시간대별 반복(§2.3).
