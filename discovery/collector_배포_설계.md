...
> 반입→실행→회수→정리하고, 데모가 매번 그 경로로 돌아 상시 검증된다. **아직인 것은 번들 생성·서명**
> (릴리스 워크플로·`pqcota-verify-bundle`)이며 [로드맵](../RELEASE_NOTES.md)에 있다. 그때까지 사용자도
> 소스에서 빌드한다 → [루트 README · 빌드](../README.md#빌드).
...
- **collector는 기존 substrate(Ansible·Salt 등)에 얹혀 돈다** — 노드 도달은 이미 그쪽이 푸는
  문제라 자체 원격 실행 엔진을 두지 않는다 ([아키텍처 §2.3](../docs/PQC플랫폼_아키텍처_및_OSS경계_설계.md)).
...
> 시점에 collector 3종만 반입한다. 노드에 중앙 도구를 뿌리면 발자국과 공격면만 커진다(§2.4).
...
| **커널 하한** | **미확정.** Go 툴체인이 정하며 툴체인 버전이 오르면 함께 오른다. **컨테이너로는 검증 불가**(호스트 커널 공유) — 실기 확인이 필요하다 |
| **기능별 요구** | `/proc` 마운트(정찰) · **`CAP_NET_RAW`**(netcap) · 시그널 권한과 `/proc/<pid>/root`(네이티브 attach) · **SELinux/AppArmor/seccomp**가 이들을 막을 수 있음 |

> **커널 하한이 왜 중요한가** — 관측 대상이 **레거시 서버**다. RHEL 6(커널 2.6.32)급 장비가 정확히
> OpenSSL 1.0.x를 물고 있을 후보인데, 최신 툴체인으로 빌드한 바이너리가 거기서 안 뜰 수 있다.
> 가장 관측이 필요한 곳에서 못 도는 상황이라 릴리스 전에 반드시 확정해야 한다.
...
- **`pqcota-netcap`은 `CAP_NET_RAW`** 필요(`setcap` 또는 root). 나머지는 일반 권한으로 동작하되,
  `/proc` 커버리지는 UID에 달렸다(root면 전 프로세스, 아니면 자기 것만 — §2.7로 고지).
