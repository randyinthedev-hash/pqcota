...
| **unit(linux)** | 순수 로직인데 **대상 코드가 리눅스 전용**이라 다른 OS에서는 컴파일되지 않는다 | 리눅스 | 〃 |
| **integration** | 실물 바이너리/JVM/프로세스 필요 | **리눅스** | 실제 OpenSSL·JDK 등 |
| **e2e** | 컨테이너 등 환경 조립 | 리눅스 + Docker | [데모](../demo/integration-verification.md) |

**`unit(linux)`은 세는 자리에서 integration으로 잡힌다.** [테스트 맵](../docs/test-map.md)이 통합의 기준을 `//go:build linux` 태그로 두었고 `check-docs`가 그 태그로 세기 때문이다. 파일 단위로 세는 규칙이라 같은 파일에 든 순수 로직까지 함께 넘어간다. 라벨을 integration으로 바꿔 수와 맞추지는 않는다. 그러면 실물이 필요 없는 케이스가 필요한 것처럼 읽혀, 무엇을 확인했는지가 흐려진다.

**반대쪽으로도 어긋난다.** TD-NETWORK-17·18은 실 crypto/tls와 실물 sshd를 쓰는 integration인데, 그 코드가 리눅스 전용이 아니라 태그가 없어 **unit으로 세어진다**. 태그를 붙여 수를 맞추지도 않는다. 그 테스트는 macOS에서도 돌기 때문이다. 그래서 **레벨 분포의 수는 태그가 만든 근사치이지 이 표의 라벨을 센 것이 아니다.**
...
| [TD-JVM-10](collectors/jvm/procscan_test.go) | unit | `TestDeriveJavaHomeWindows`·`TestIsJavaExeWindows`: Windows 경로 규칙 | `...\bin\java.exe` → JAVA_HOME, `bin`이 없으면 **빈 값** | 경로 규칙은 순수 문자열 처리라 리눅스 CI에서 검증한다. 실제 장비 없이 못 잡는 자리를 여기서 못 박는다 |
| TD-JVM-11 | **실물로 확인** | Windows 11(26200)에서 `pqcota-jvmscan --recon` | 프로세스 255–261개를 훑고, `\fakejdk\bin\java.exe`로 뜬 프로세스를 잡아 `javaHome`을 `\fakejdk`로 낸다. `attachCapable`=false·`version` 없음 | 교차 컴파일은 API 이름만 보증한다. Toolhelp32가 실제로 무엇을 돌려주는지, 경로 규칙이 실제 장비와 맞는지는 돌려 봐야 안다 |
| TD-JVM-12 | **실물로 확인** | 같은 명령을 일반 사용자와 관리자로 | 일반 265개 중 **163개를 못 열고**, 관리자는 264개 중 **3개** | Windows에서 Java 서버는 보통 서비스(SYSTEM)로 돈다. 권한 없이 돌리면 봐야 할 JVM이 통째로 안 보인다. 숫자만 내면 "JVM 0개"로 읽히므로 화면이 뜻과 넓히는 법을 함께 낸다 |
| [TD-JVM-13](collectors/jvm/attach_test.go) | unit | `TestDegradedNoteNamesTheJVM`·`TestDegradedNoteCarriesItsOwnReason`·`TestGapResultCarriesTheJVMToTheCentre` | 갭 노트가 **어느 JVM·왜**인지 밝히고, 찾았는데 못 본 JVM은 컴포넌트 없이 갭만 실어 보낸다 | 셋 다 실제 장비에서 드러난 결함을 못 박은 것이다(아래) |
| TD-JVM-14 | **실물로 확인** | Windows 11 + JDK 21에서 `pqcota-jvmscan --output table` | ①이 실패하고 **②가 붙는다**(대상 JVM이 `A Java agent has been loaded dynamically`를 찍는다). javapath 런처 심은 `jvm.dll not loaded by target process`로 **갭**이 되고, 그 사유가 화면과 계약에 남는다 | Windows attach 경로의 첫 실물 확인이다. 여기서 결함 셋이 나왔다: ②가 실패 시 **클라이언트 자신의** java.security를 읽어 남의 provider를 대상에 붙였고, 도는 JVM이 없으면 프로브가 **띄운 JVM**을 confirmed로 냈으며, 관측 못 한 JVM이 **중앙에 가지 않았다** |
...
> 오프라인 **번들 생성**(턴키 배포)은 하지 않는다. 여기 테스트는 **임포트 시 provenance 서명 검증**(수신 측)만 한다.
...
> 노드 식별(TD-CNG-7)이다. 고친 뒤 재측정으로 둘 다 닫았다.
...
| TD-CNG-11 | **실물로 확인** | Windows 노드에서 `hardware_uuid` 관측 | `0CA88DB0-…`가 나오고 **Windows 자신이 보고하는 값과 같다**(`Win32_ComputerSystemProduct.UUID` 대조). `derived_from`은 `machine-id` 그대로 | SMBIOS는 앞 세 묶음이 리틀엔디언이라 되돌리지 않으면 같은 머신이 듀얼 부팅에서 다른 UUID로 보인다. 그리고 지문이 하나 늘어도 **node_id는 흔들리지 않아야** 한다(우선순위가 machine-id 먼저) |
| TD-CNG-12 | **실물로 확인** | 알고리즘마다 `BCryptEnumProviders` | `ML-DSA`·`ECDH_P256`·`SHA256` 모두 `Microsoft Primitive Provider`. **50개 전부 provider가 하나씩** | 등록 목록은 "무엇이 있나"만 답한다. 조치 대상을 고르려면 **누가 그 알고리즘을 서비스하나**가 필요하다. 그리고 겹치는 provider가 없다는 것은 우선순위 전제가 CNG에서 서지 않는다는 뜻이다 |
| TD-CNG-9 | **실물 종단**: 확인 | 실제 장비 결과를 `pqcota-ingest` → `pqcota-inventory` | `win_cng  confirmed  runtime_introspection  providers=9 algorithms=50 네이티브(서명만: KEM 미관측)` · 이력 1건 | 관측이 화면까지 오지 않으면 적지 않은 것과 같다. 파생까지만 보고 닫으면 그리는 자리가 비어 있는 것을 못 잡는다. 실제로 뷰 둘이 비어 있었다 |
...
**관찰**: 순서 1–7이 전부 **unit**이다. 핵심 로직(정직한 증거·fork·라우팅·위임경계)이 실물 없이 TDD된다. 실물 의존은 리눅스가 필요하다. **가치 있는 로직을 먼저, 환경 리스크는 조기 PoC로.**
