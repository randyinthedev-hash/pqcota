...
// json 태그는 raw_capture(§2.5 step 1)에 실리는 형식이다 — 필드 이름을 바꾸면 과거 포집을
// 재정규화할 수 없게 되므로 raw_format을 함께 올린다("openssl-collector/native-v1").
type Detection struct {
	Lib             string   `json:"lib"` // libssl | libcrypto
	Path            string   `json:"path"`
	Fork            string   `json:"fork,omitempty"` // registry.MatchFork 결과 ("" = unknown, §2.6)
	...
	AppKeys         []string `json:"appKeys,omitempty"`         // 자산 귀속(§4A.2) — 이 .so를 로드한 앱(들). host-wide 스캔에서 dedup 시 합집합.
}

// RawCapture — 탐지 결과를 collector 네이티브 형식(JSON)으로. CycloneDX 변환 **전**의 원본이라
// 강화 규칙이 좋아지면 재수집 없이 여기서 다시 정규화한다(§0.2·§2.5 step 1).
// mergeByPath가 순서를 고정하므로 같은 관측이면 같은 바이트다(서명이 이 값을 덮는다, §2.7).
...
func DetectForPID(pid int, sigs []registry.ForkSignature) ([]Detection, error) {
	...
	appKey, _ := procs.AppKey("/proc", pid) // 이 PID의 앱 귀속(§4A.2) — cgroup systemd 유닛 or exe
	var appKeys []string
	if appKey != "" {
