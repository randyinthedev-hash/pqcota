...
type Detection struct {
	...
	AppKeys         []string `json:"appKeys,omitempty"`         // 자산 귀속(§0.5) — 이 .so를 로드한 앱(들). host-wide 스캔에서 dedup 시 합집합.
}
...
func DetectForPID(pid int, sigs []registry.ForkSignature) ([]Detection, error) {
	...
	appKey, _ := procs.AppKey("/proc", pid) // 이 PID의 앱 귀속(§0.5) — cgroup systemd 유닛 or exe
	var appKeys []string
	if appKey != "" {
