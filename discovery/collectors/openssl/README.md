...
**설계 목표**: 그 호스트에 *설치된* 것이 아니라 **실행 중인 프로세스가 실제로 로드한** libssl/libcrypto를 본다. 패키지 목록은 "깔려 있다"까지만 말하지만, 마이그레이션이 알아야 하는 것은 **무엇이 지금 쓰이고 있고, 그것을 바꾸면 어느 앱이 영향받는가**다.

> **§ 표기**: 별도 언급이 없으면 [규정서](../../../docs/regulation.md)의 절 번호다.

근거: [디스커버리 설계 §2.1](../../design.md) · 인수 기준: [테스트케이스 SD-1·SD-3·SD-4](../../testcases.md)
...
| **어느 앱 것인가**(`app_keys`) | 어느 프로세스가 로드했나 | 공유 `.so` 하나를 여러 앱이 쓰면 **여러 앱에 걸침**: 교체가 미치는 영향 범위 |

## 외부 도구에 의존하지 않는다

`/proc`과 ELF를 **Go로 자체 파싱**한다(`debug/elf`). `ldd`·`lsof`·`ss`·`readelf`를 부르지 않는다.

레거시 서버는 최소 이미지이거나 도구가 없거나 버전이 제각각이다. 외부 도구에 기대면 **관측 실패가 환경 탓으로 흩어져** 무엇을 관측하지 못했는지조차 불분명해진다. 자체 파싱이면 노드에 올라가는 것이 정적 바이너리 하나로 끝나고, 실패 원인도 collector 안에서 통제된다.

## 정직성: 관측하지 못한 것을 "없다"로 만들지 않는다

- **fork를 못 가리면** 빈 문자열로 두고 `evidence_strength=INFERRED_LOW`로 낮춘다. 추정해서 채우지 않는다(TD-FORK-2).
- **대상이 실행 중이 아니면** 자산 0이 아니라 **PROCESS 계층 갭**으로 보고한다. "안 쓴다"가 아니라 "이번 관측 구간에서 보지 못했다"(§2.6).
- 네임스페이스가 분리돼 `/proc`이 안 보이면 역시 **갭**이다(TD-CONTAINER-2). 수집 실패를 「관측 0건」으로 바꾸지 않는다.
...
- **리눅스 노드 전용**: `/proc`와 ELF에 기댄다. **빌드 태그로 가르지는 않아서**(netcap과 다르다) 다른 OS용으로도 컴파일되고, 돌리면 빈 결과가 아니라 **갭**을 낸다. "OpenSSL 없음"이 아니라 "관측하지 못함"이다(§2.6). Windows에서의 관측은 [로드맵](../../../RELEASE_NOTES.md#로드맵-예정-릴리스-계획)에 있다.
