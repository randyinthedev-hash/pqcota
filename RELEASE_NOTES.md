...
- **v0.7.0 (계획)**은 **CNG 프로비저닝**이다. **substrate 일반화가 선행한다**(POSIX 파일 가정을
  벗어난다. Windows는 레지스트리/GPO라 `/opt/pqcota` 파일 스테이징·파일제거 롤백에 안 맞는다) → `renderCNG`. 일반화는 이 구현과
...
- **Windows OpenSSL 관측 (계획 · 버전 미정).** 지금 `pqcota-nodescan`은 구현이 하나뿐이고 `/proc`를
  ...
  문자열을 뽑는 곳(`elfstrings.go` → PE)이다. **fork 판별(`registry.MatchFork`)은 뽑아낸 문자열만 받으므로
