...
- **통신 관측 주기 분리 (계획 · 버전 미정).** 자산과 통신 엣지는 관측 주기가 다르다. 지금은 한
  ...
  `first_seen`·`last_seen`은 아무도 채우지 않는다. 그리고 **병합 키가 세 곳에서 다르다.** 정규화 키에
  협상 그룹이 들어 있어 「1회차에 알아낸 그룹, 2회차에 미상」이 다른 엣지 둘이 되고, 같은 키에 포트가
  없어 상대가 노드로 해소되면 443과 8443이 한 엣지로 접힌다.
...
- **엣지 상대의 스코프 판정 (계획 · 버전 미정).** 관측이 스코프 마스터에 없는 노드를 만나면 **등재 판정
  ...
  운영자가 등재하지 못한 노드를 관측으로 드러내는 유일한 자동 수단이라 아쉬운 자리다.
...
**목표**: v0.6.0을 내고 남겨 둔 두 빈칸을 채운다. 둘 다 **실기 한 번으로 함께 확인**했다.
