...
- **v0.2.0 (계획)** — **적재 경로를 여럿이 쓰는 전제로 옮긴다.** 지금 인벤토리는 한 조직·한 실행을
  전제로 돈다. 아래 여섯은 그 전제가 깨지는 자리들이고, 전부 **이 리포가 이미 지키는 원칙 — 조용히
  빠지면 "없다"로 읽힌다 — 을 적재 경로에도 적용하는 일**이다.
  - **조직 축.** `node_id`가 전역 키라 다른 조직이 같은 이름을 쓰면 이력이 하나로 합쳐진다. 저장소
    핸들을 조직에 묶고(질의마다 기억할 일이 없으니 잊을 일도 없다) 인덱스 선두를 org로 둔다.
    기존 생성자는 기본 조직에 묶어 그대로 두고 새 생성자를 더하므로 **API는 깨지지 않고, 단일 조직
    사용자는 조직을 대지 않는다.** 필수 모드에서는 기본 조직이 에러가 된다.
  - **DDL 자동 실행 안전장치.** 지금 `NewPgStore`가 생성자에서 말없이 `CREATE TABLE`을 돈다.
    가리키는 곳이 어긋나면 빈 테이블이 새로 생기고 거기에 쓴다 — 조용히 실패한다.
    스키마 배포는 의도적 행위여야 한다.
  - **거절·오프스코프·신원충돌을 이력으로 남긴다.** 지금 `IngestReport`는 반환값이라 화면에 찍히고
    사라진다 — 절단 사실을 남기는 `pqcota_retention_events`와 같은 이유다. 남기지 않으면 "잘못
    보내졌다"와 "아무 일도 없었다"가 구분되지 않는다.
  - **서명 검증 필수 모드.** 지금은 검증자가 없으면 조용히 통과한다(전송 보안이 대신한다는 전제).
    그 전제가 서지 않는 경로를 위해, 검증하지 않은 건수를 보고하고 필수 모드에서는 끊는다.
  - **공개키를 collector에 묶어 검증한다.** 지금 `Verify`는 넘긴 키를 **전부** 시도하고 어느 키가
    맞았는지 묻지 않는다. `collector_id → 공개키`로 대조하는 형태를 더한다(기존 `Verify`는 그대로).
  - **`raw_capture` 규약을 계약 주석에 못 박는다** — 파일 원문·패킷 페이로드·자격증명을 담지 않는다.
    지금은 collector의 규율이 지킬 뿐 계약에 적혀 있지 않다.

- **v0.3.0 (계획)** — **엣지를 앱에 귀속**: 지금 `ObservedEdge`는 노드까지만 간다. 한 노드에서 두 앱이
...
- **v0.4.0 (계획)** — **CNG 디스커버리**: Windows 콜렉터(`BCryptEnumProviders`·레지스트리 인트로스펙션)로
  `CngAxes`를 채워 자산이 인벤토리에 수렴. (스키마는 v0.1.0에서 이미 예약 — 이 릴리스는 "채우는 코드".)
  설계 검토: [검토 중인 설계 §2.2](docs/under-review.md).
- **v0.5.0 (계획)** — **CNG 프로비저닝**: **substrate 일반화 선행**(POSIX 파일 가정 탈피 — Windows는
