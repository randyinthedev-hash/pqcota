...
- **v0.8.0 (계획)**은 **CNG 프로비저닝**이다. v0.7.0을 계획 수신·실행 관문 정비에 썼으므로 한 칸 뒤로 간다. **substrate 일반화가 선행한다**(POSIX 파일 가정을
...
## v0.9.0: 조치가 어느 스냅샷 상태에서 나왔는지 되짚는다 (작성 중)
...
- **조치가 근거를 든다.** `RemediationAction.evidence_sources[] = {finding_id, snapshot}` —
  어느 finding이, 어느 스냅샷 상태에서. 참조(`SnapshotReference`)는 이력의 실제 id이거나 내용
...
- **생성기가 참조를 실제로 찾는다.** 모양은 `--dsn` 없이도 보고(틀리면 종료 3), `--dsn`이면
  ...
  원천 노드 이름·규칙 판·스코프 정책·결과 집합)를 값으로 말한다. `pqcota-records`가 `snapshot:` 줄로
  보인다. 레코드 → 계획 → 스냅샷 사슬이 이것으로 닫힌다.

### 고친 것

- **노드별 병합이 입력 순서에 기대고 있었다**(v0.1.0–v0.8.0). 같은 finding·같은 엣지·완전성 note
  모두 「먼저 온 것이 이긴다」였다. **잘못 나온 것**: 결과 파일의 순서가 바뀌면 스냅샷이 달라졌다.
...
- **중복 억제가 옛 지문으로 접었다**. v1 열을 두고도 옛 지문으로 계속 접었다면 v1이 빈 옛 행이
  ...
  **업그레이드 뒤 첫 적재는 같은 상태여도 새 행이 생긴다** — 이행의 자국이지 변화가 아니다.
...
- **지금 판의 복사본만 잡고 있었다**(v0.7.4–v0.7.5). 상수가 `pqcota-enrich/v2` 로 올라가면,
  ...
  **옛 판을 찍는 코드가 조용히 남고**, 그때부터 그 스냅샷들은 지금 규칙으로 만들어진 것처럼
...
> 넘기려면 `--allow-incomplete`. ③ 모르는 `--level` 값은 이제 **종료 2**다(전에는 조용히 L2였다).
...
- **확인할 키가 없으면 거절한다.** 전에는 경고하고 통과시켰고 닫는 것은 `PQCOTA_REQUIRE_APPROVAL=1`을
  건 배포에서만 일어났다. 그러면 승인 무결성이 「닫을 수 있는 수단」에 머문다. 알고 여는 길은
  `--allow-unverified-approvals` 하나이고 **명령줄에 적어야 열린다** — 환경변수로도 열리면 무엇이
  검증됐는지가 셸 설정에 숨는다.
...
- **위임 수준을 말하지 않은 계획은 빈칸으로 센다.** 말하지 않으면 실행 수준이 `--level`에서 오는데,
  그 플래그는 **승인 서명 밖에 있다** — 서명은 계획을 덮지 계획을 부르는 명령줄을 덮지 않는다.
...
- **빈칸이 남은 계획은 종료 상태 3이다.** 산출물은 그대로 낸다 — 사람이 손으로 채우는 것이 정당한
  경로라 막지 않는다. 다만 성공으로 끝내지 않는다. 알고 넘길 때는 `--allow-incomplete`.
...
- **게이트 다섯을 더했다.** 규칙을 적어 두고 제품이 부르지 않는 자리, 문서의 Go 버전과 `go.mod`,
  테스트 레벨 수, 관리체계의 숫자, 한국어 문서와 영문 짝의 뼈대다.
...
- **환경이 정반대인 케이스는 한 번에 재지 못한다.** TD-NETWORK-15는 `CAP_NET_RAW`가 있어야, 16은
  없어야 돈다. 한 러너에서 둘을 다 재는 길이 없어 두 번 돌린다.
...
- **모르는 `--level` 값이 조용히 L2로 돌았다**(v0.1.0–v0.6.7). `--level L3`처럼 대소문자만 틀려도
