...
- **`pqcota-declare-attribution`** — CSV(`node_id,dst,port,app_key`)를 선언 레인 결과로 임포트한다.
  어느 엣지를 가리키는지 모르는 줄(포트가 숫자가 아님·app_key 없음·node_id 없음)은 **추측하지
  않고 멈춘다** — 틀린 앱에 귀속하면 조치 대상이 바뀐다.
- **`AttributionOverlay`** — 선언과 관측을 **읽을 때만** 합친다. 관측이 이미 채운 자리는 그대로
  두고 빈 자리만 메우며, 메운 것은 `@app(declared)`로 나오고 몇 건이 선언인지도 함께 밝힌다.
  색인은 `app_key_kind="declared"`인 엣지만 받으므로 관측이 관측을 메우는 일이 없다.
