...
- **v0.6.0 (계획)** — **CNG 디스커버리**: Windows 콜렉터(`BCryptEnumProviders`·레지스트리 인트로스펙션)로
...
- **낱말 하나를 바꾸면 네 곳이 함께 움직인다.** 영어를 옮겨 온 어휘를 우리말로 풀었는데
  (`관측 창`→`관측 구간` · `실기`→`실제 장비` · `부모 사슬`→`부모 체인` · `귀속`→`어느 앱인지
  밝힌다` · `posture`→`등급`), 산문만 고치면 **문서와 화면이 같은 것을 다르게 부른다.** 화면에
  나가는 문자열, 그것을 담은 샘플 산출물, 그 출력을 `grep`·`sed`로 긁어 쓰는 스크립트와 테스트가
  함께 움직여야 한 벌이 된다.
- **식별자는 따라 바꾸지 않는다.** `procs.Attribution`·`pqcota-declare-attribution`·
  `pqcota_edge_attribution`·`pkg/kernel/posture`는 그대로 뒀다 — 릴리스된 CLI·테이블·패키지
  이름을 문구 정리로 바꾸면 그건 문구 정리가 아니다.
