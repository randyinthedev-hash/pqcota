...
- **v0.4.0 (계획)** — **귀속의 선언 레인**: 자동 경로가 원리상 못 보는 자리를 사람이 채운다.
  v0.3.0 데모에서 **엣지 4개 중 3개를 못 잡았다**(짧은 연결). 그 트래픽이 설계상 전부 짧아 최악에
  가까운 값이지만, 짧은 연결 자체는 예외가 아니다 — 배치·헬스체크·cron·SSH가 그렇다. 입력은
  `pqcota-declare`가 이미 쓰는 레인으로 받아 관측과 섞지 않는다:
  [검토 중인 설계 §5.2](docs/under-review.md).
