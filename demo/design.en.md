...
- **One machine registered under several names is reported**: the ingest output carries `⚠ duplicate: physical machine … → [pay-db web-gw]`, and that is not an error. The platform states that the fingerprints match instead of hiding it (TK-MACHINE). This is what you see in production when one machine has been registered under several names.<br>**The line depends on the host.** The fingerprint is picked in the order `cloud-instance-id` → `/etc/machine-id` → the DMI `product_uuid` → the hostname (`pkg/kernel/machineid`). A container's `/etc/machine-id` is empty, so the DMI entry decides. **On a Linux host**, the host's `/sys/class/dmi/id/product_uuid` is visible from inside a container, so all three nodes carry the same fingerprint and the line appears. **On Docker Desktop for Mac**, that VM has no DMI, so the value is empty and the fingerprint falls to the hostname (the container name), which differs per node — and the line does not appear. It is read, not invented (§2.5).
...
- **History**: the same retrieved results are ingested a second time (the equivalent of "the next scan round" in production). The content is identical, so **no new snapshot is created** and only one observation record is added. `-history` shows that as one point of change with two observations, and `-snapshot` unfolds that snapshot's assets and observed edges. Storage grows with the number of changes, while the evidence that "we scanned every time" is preserved. The tool does not invent change that is not there.
- **Change**: the two points `-diff` compares are **before and after the asset scope**. Once the scope drops the noise the content differs, a new snapshot is created, and comparing the two yields one `removed` in the default topology. That `removed` means **dropped from management**, not that the asset is gone. When a version really does change, the finding id is preserved and it shows up as **a `changed` on the same asset**.
