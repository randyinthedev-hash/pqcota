...
- **같은 장비가 여러 이름으로 등재되면 고지한다**: 적재 결과에 `⚠ duplicate: physical machine … → [pay-db web-gw]`가 나오는데 이것은 오류가 아니다. 데모의 타깃들은 한 호스트 위의 컨테이너라 `pay-db`와 `web-gw`의 물리 장비 지문이 같고, 플랫폼은 그 사실을 감추지 않고 그대로 알린다(TK-MACHINE). 실운용에서 한 장비를 여러 이름으로 등재했을 때 보게 되는 표시가 이것이다.
