...
데모를 **스크린캐스트로 만들 때**의 절차와 틀을 적는다. 발표·블로그·대회 출품 어디에나 같다.
...
터미널을 화면 녹화해도 되지만, `asciinema`로 녹화하면 **찍은 뒤에도 폰트·테마·배속을 바꿀 수 있다.**
...
- **정지 프레임은 그 구간의 마지막에서 캡처한다.** 앞에서 캡처하면 이미 인쇄된 줄이 사라졌다
  다시 나타난다. 영상을 조작한 것처럼 보인다.
- **제작 시기를 넣는다.** 화면의 버전·provider는 시간이 지나면 달라진다. 언제의 관측인지가
  근거의 일부다.
- **강조는 확대가 아니라 밑줄·테두리.** 터미널 클립을 확대하면 글자가 뭉개진다.
- 컷 경계는 화면의 구분선(`────`)이다. `.cast`에서 그 시각을 읽으면 자를 지점이 그대로 나온다.
...
이 폴더의 나머지 파일은 **틀**이다. 그대로 쓰지 말고 자기 작업 폴더로 복사한 뒤 내용을 바꾼다.
...
켜지는 구조라 녹화할 이유가 없다. 헤드리스로 각 상태를 캡처하면 커서도 흔들림도 없고, 다시 만들어도
...
캡처한 PNG를 `ffmpeg`의 `xfade`로 이으면 도입 클립이 된다(각 상태를 몇 초 보일지는 대본에서 정한다).
...
것은 분량도 강조할 곳도 다르다. 그래서 완성본 대신 틀만 둔다.
