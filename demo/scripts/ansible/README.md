...
| `groups.ini` | **그룹 멤버십 + 트래픽 시나리오만**(접속 정보 아님). 노드별 `traffic=`(관측 창을 채울 핸드셰이크 대상).<br>**생성물** — `demo/topology/topology.yaml`에서 `topogen`이 만들어 `demo/.generated/`에 둔다(직접 편집하지 않는다) |
...
- `groups.ini`의 `traffic="pqc:host:port ssl:host:port ssh:host:port"` — 관측 창 동안 생성할 핸드셰이크 대상(없으면 `traffic=""`).
