...
#   observe    정적 스캔으로는 못 보는 provider를 런타임 관측이 잡는다
#   provision  도구가 만든 config가 실제 암호 능력을 만들고, 되돌리면 원상복귀한다
#   gap        못 본 것을 "없음"으로 적지 않는다
#
...
# ─────────────────────────────────────────────────────────────────────────────
# 컷 1 — 정적 스캔이 못 보는 것을 런타임 관측이 잡는다
#
...
# ─────────────────────────────────────────────────────────────────────────────
# 컷 2 — 도구가 만든 config가 실제 암호 능력을 만들고, 되돌리면 원상복귀한다
# ─────────────────────────────────────────────────────────────────────────────
take_provision() {
	need_ctl
...
print(json.dumps(d.get(\"completeness\",{}), ensure_ascii=False, indent=1))"'
	...
	note "   인벤토리에는 '이 노드엔 링크가 없다'로 읽힌다. 없는 것과 못 본 것은 다르다."
}
