...
- `⚠ duplicate: physical machine … → [pay-db web-gw]`: this appears **on a Linux host**. The demo targets are containers on one host, and on Linux the host's machine fingerprint is visible from inside a container, so all three carry the same value. It is what you see in production when one machine has been registered under several names. On Docker Desktop for Mac the fingerprint comes from elsewhere, so the line does not appear ([why](design.en.md#52-the-central-inventory-endpoints-profiles-app-attribution-history-change)).
