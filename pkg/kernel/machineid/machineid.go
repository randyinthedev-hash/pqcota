// Package machineid — 머신 상관 지문 수집 + 자동 UID(self-id) 결정론적 부여 (규정서 §0.4).
...
// SelfAssign — 지문에서 결정론적 node_id를 파생한다(§0.4 self-id). CMDB 미등재 폴백.
...
// Fingerprint — 호스트에서 안정 지문을 best-effort 수집한다(linux). 없는 건 빈 값으로 남긴다(§2.6 정직).
