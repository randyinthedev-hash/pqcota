...
// 못 쓰게 된다. 무엇을 계속 볼지는 **사용자가 선언**하고, 도구는 그 선언을 집행한다(§0.1 —
// 판단은 마에스트로, 집행은 단원).
//
// ★ 제외는 "없음"이 아니다. 정책으로 뺀 자산을 조용히 사라지게 하면 인벤토리는 "그런 게
// 없다"고 거짓말한다 — §2.7이 금지하는 바로 그것이다. 그래서 Apply는 제외분을 **세어서
...
//
// ★ 공유 .so 주의: 귀속 앱이 여럿이라(§4A.2) 한 앱만 겨냥해 exclude해도 그 .so를 함께 쓰는
