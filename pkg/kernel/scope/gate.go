...
// Master — 스코프 마스터(등재 노드 집합). 관리 대상 경계의 유일 권위(§1.4).
type Master struct {
...
// Gate — 수집 대상 후보를 등재분(allowed)과 미등재분(rejected)으로 가른다(§1.4).
...
// ObservedDisposition — 수집 중 관측된 노드의 처리(§1.4, SD-5).
...
// ClassifyObserved — 수집 중 관측된 노드를 분류한다. 미등재면 등재 판정 요청으로(§2.5).
