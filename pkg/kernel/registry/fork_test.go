...
func TestMatchFork(t *testing.T) {
	...
	t.Run("시그니처 없음 → unknown 명시", func(t *testing.T) {
	...
	t.Run("AWS-LC 동일 soname 구분(OpenSSL로 오분류 금지)", func(t *testing.T) {
	...
	t.Run("OpenSSL 버전 추출", func(t *testing.T) {
	...
	t.Run("버전은 배너 문자열에서 (이름만 있는 문자열 먼저 나와도)", func(t *testing.T) {
