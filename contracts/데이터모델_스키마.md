...
4. **비밀 미영속(§0.5)** — 접근 비밀(SSH 키·비밀번호·계정)은 **어떤 스키마에도 필드가 없다**. `MachineEndpoint`가 대표 — 타입상 비밀을 담을 수 없어 컴파일 타임에 보장된다.
...
| `QuantumPosture` | 양자내성(§6.1). **파생 뷰** — core가 `negotiated_group`에서 분류 | 🟢`PQC_HYBRID`·🔴`CLASSICAL`·⚪`UNSPECIFIED` |
...
| **`MachineEndpoint`** | discovery 재접속용 **재사용 연결 메타데이터** | `node_id`·`name`·`ip`·`port` — ★**비밀 필드 없음**(키·계정·암호는 사용자 파일에만, §0.5) |
