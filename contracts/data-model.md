...
|---|---|---|
| `CryptoRuntime` | 암호 런타임 — 무엇을 받는지는 [수용 원칙](../docs/runtime-acceptance.md). 모든 finding·자산·조치의 1급 분기 | `OPENSSL` · `JCA` · `WIN_CNG`(v0.1.0 **스키마 예약**, 미구현 — 채우는 코드는 v0.3.0+) |
...
core는 "노드를 주면 정규화된 CBOM을 반환한다"는 추상만 의존한다. 이 gRPC 경계가 곧 GPL 전염 차단 경계(라이선스 정리).
...
core 정규화 파이프라인이 `cbom_cyclonedx` 본문에서 파생하는 타입드 뷰. **재계산 가능**(§1.2).
...
| `CngAxes` | Windows CNG 분기축 (**v0.1.0 예약**, 미구현) | `provider_set`(KSP/SSP, **순서 유의미**). 실물 관측이 정할 나머지 필드는 v0.3.0에서 additive로 추가 |
...
> 두 관측은 `node_id`에서만 만난다 — 잇는 일은 [v0.2.0 예정](../docs/under-review.md)이다.
