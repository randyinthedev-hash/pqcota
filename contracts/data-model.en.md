...
|---|---|---|
| `CryptoRuntime` | the crypto runtime — what is accepted is in the [acceptance principles](../docs/runtime-acceptance.en.md). The first-class branch for every finding, asset, and remediation | `OPENSSL` · `JCA` · `WIN_CNG` (**schema reserved** in v0.1.0, unimplemented — the code that fills it comes in v0.3.0+) |
...
| `CngAxes` | the Windows CNG branch axis (**reserved in v0.1.0**, unimplemented) | `provider_set` (KSP/SSP, **order is meaningful**). The remaining fields, which real observation will settle, are added additively in v0.3.0 |
...
> ([under review — attributing edges to apps](../docs/under-review.en.md), planned for v0.2.0).
