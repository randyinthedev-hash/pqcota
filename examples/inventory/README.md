...
> **샘플 형태 주의**: 현재 정규화는 CBOM의 **`pqcota:` 프로퍼티**를 읽는다([`sample-cbom.json`](sample-cbom.json)이 그 형태 — JCA/BouncyCastle을 소스에서 발견한 셈). CBOMkit 표준 출력(`cryptoProperties`)을 pqcota 스키마로 매핑하는 것은 import 어댑터의 **확장 지점**이다 — 지금은 pqcota-매핑된 CBOM만 자산으로 파싱된다. 상세: [위임 수신 설계](../../inventory/위임수신_설계.md).
