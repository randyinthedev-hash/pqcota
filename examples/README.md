...
## 전제
- **Go 툴체인 + buf** — 클론 직후 리포 루트에서 `make generate`를 한 번 돌린다(`gen/`은 커밋되지 않는다, [CONTRIBUTING](../CONTRIBUTING.md)). 그다음은 `go run`으로 소스에서 직접 실행하며 Postgres·타깃 노드는 필요 없다.
- **예외 — [discovery/jvm](discovery/jvm/README.md)**: 정찰→attach는 **살아있는 JVM**이 필요해 **Docker**를 쓴다(JDK는 컨테이너 안에 있다). 아래 표에 표시.
