...
**점선 하나가 이 도구의 성질이다.** 한 노드만 관측하려면 2·4를 건너뛰고 화면에서 끝난다. 중앙도
데이터베이스도 필요 없다.
...
두 방식은 배타적이지 않다. 같은 collector가 같은 `CollectionResult`를 내고, 그것을 화면으로 볼지
쌓을지가 갈릴 뿐이다.
...
| 관측하지 못한 것은 | **못 봤다고 적는다.** 완전성 맵이 그것을 적는 곳이다(§2.6). 고지 없이 빠지면 *"그 노드엔 없다"*로 읽힌다 |
...
| | |
|---|---|
| **Go 1.26.4+ · buf** | 두 가지를 만든다. 컨트롤러에서 쓸 CLI와 노드에 올릴 collector다 |
...
리눅스 노드 요건은 **커널 3.2 이상**뿐이다. 이는 Go 툴체인이 정하는 하한이고, 정적 링크라 배포판·libc를
...
|---|---|---|
| **nodescan** | 로드된 libcrypto/libssl · 버전 · fork · 어느 앱이 쓰나 | `/proc`·ELF를 **자체 파싱**한다. `ldd`·`readelf`를 부르지 않는다 |
...
그 일을 겹쳐 만들 이유가 없다. 대신 CI가 낸 **표준 CycloneDX를 받아** 검증·정규화해서
...
| | |
|---|---|
| 거부하는 것 | **둘뿐이다.** 구조 부적합(CycloneDX가 아니거나 미지원 `specVersion`)과 서명 검증 실패다. 기계적으로 판정되는 것만 막는다 |
...
| 관문 | 켜는 법 | 막는 것 |
|---|---|---|
| 노드 등재 | `[scope-master-file]` | 미등재 노드다. 버리지 않고 **등재요청**으로 남긴다 |
...
append-only 레코드로 남긴다. 되돌릴 때 무엇으로 돌아가야 하는지를 밝히는 근거가 여기 있다.
...
나온다. 레코드를 읽지 않는다. 레코드는 무엇으로 돌아가야 하는지를 밝히는 **근거**이지 되돌림의 입력이 아니다.
...
| **조치 뒤 확인** | 인벤토리가 그대로면 그 이유까지 함께 낸다. 관측 경로가 없는 층인지, 상대가 옛 스택인지를 밝힌다 |
| **보존 절단** | 오래된 변화 지점을 자르고, 자른 사실을 기록으로 남긴다 |
| **제외분 고지** | 스코프로 뺀 자산은 사라지지 않고 건수가 화면에 남는다 |
...
| 6 · 확정 계획을 무엇으로 채우나 |: | **정하지 않는다.** 사용자가 직접 작성한 계획을 그대로 입력으로 받는다 |
| 화면(UI) |: | **없다.** CLI와 생성물이다 |
