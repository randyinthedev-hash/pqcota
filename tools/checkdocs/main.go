...
// 적고, 누가 하느냐는 적지 않는다. 계획은 로드맵에 적는다.
//
// ★ 왜 셸(grep)이 아닌가 — BSD grep은 `.`를 바이트로 세서 한글 문장에서 `{0,80}` 범위가 짧아진다.
// 같은 게이트가 Mac에서 통과하고 리눅스에서 실패했다. 로컬과 빌드 머신의 판정이 다르면 게이트가 아니다.
...
var (
	...
	//     안 하는 일은 "하지 않는다"로 적고, 계획은 로드맵에 적는다. (한때 120곳이 있었고, 그중
	...
	// §N · §1.5 · §6A 같은 절 참조. 「§ 표기」 안내문 자신은 숫자가 안 붙어 걸리지 않는다.
	...
	// 아무도 모르는 사이에 낡는다. 필요한 것은 "리눅스가 필요하다" 같은 **요건**이지 내 머신 이름이 아니다.
...
func main() {
	...
	for _, d := range docs {
		...
		// 제목에서 만든 앵커 + 문서가 직접 적은 앵커(<a id="...">) 둘 다 유효하다.
...
// 줄 단위로는 안 보인다 — 문단 전체가 표시 없이 그어진다.
...
// 머리말도 한 칸으로 둔다(거기서 어긋나는 일이 실제로 있다).
